...
\begin{flushright}
\og \an{A fundamental problem in artificial intelligence \\
	is that nobody really knows what intelligence is} \fg\ \\ 
	Shane Legg \& Marcus Hutter \cite[p.~1]{legg_universal_2007}
\end{flushright}

\noindent
Si, comme Legg et Hutter l'affirment, personne ne sait vraiment ce qu'est l'intelligence, il reste que chacun en possède une et s'en fait une certaine opinion. 
Cette conception naïve repose sur l'arrière-plan de culture philosophique et scientifique dans lequel nous nous trouvons immergés. 

Ce chapitre passe en revue les grandes lignes de cet arrière-plan du point de vue de la recherche en intelligence artificielle développementale. 
Il vise un double rôle. 
Le premier est d'établir un langage de référence largement admis dans la communauté scientifique pour parler des différents aspects de la cognition: expérience, apprentissage, connaissance, développement, attention, émotion, motivation, individuation. 
Le second est de présenter, en les organisant de manière regroupée et progressive, les sources d'inspiration et éléments de réflexion qui m'ont conduit à des propositions plus originales introduites à partir du chapitre 3.  

Ce chapitre ne répond donc pas à la question de savoir ce qu'est l'intelligence, mais il tente de résumer un large consensus philosophique et neuroscientifique sur lequel vient s'appuyer une proposition plus personnelle dans le domaine de l'intelligence artificielle développementale. 
...
\noindent
On peut facilement dire en anglais que le projet de l'intelligence artificielle est de créer un \an{artificial mind} mais on utilise rarement en français l'expression \emph{esprit artificiel} qui renverrait à d'autres connotations. 
Cette section vise à poser les bases de ce qu'on appelle \an{the mind}, comment il peut apprendre à partir de l'\emph{expérience}, comment il peut être compris dans une logique évolutionniste dans lequel s'inscrit \an{the animal's mind}, comment il peut éventuellement tendre vers une connaissance objective, et comment on peut concevoir sa liberté.   
Cette section nous conduit à définir l'épistémologie génétique et constructiviste qui fonde les théories de l'apprentissage développemental. 
...
\noindent
La manière moderne de penser le savoir, la raison, et l'expérience doit beaucoup à la révolution épistémologique des Lumières du XVIII\ieme{}~siècle.
...
Il propose la \an{bundle theory} (traduite en français par \emph{théorie du faisceau}, à ne pas confondre avec la \an{sheaf theory} ou théorie mathématique des faisceaux) selon laquelle les objets sont constitués uniquement par une collection (\an{bundle}) d'impressions dont le sujet fait l'expérience.  

En Europe continentale, le médecin et philosophe français Julien Offray de La Mettrie publie en 1748 \og L'Homme machine \fg\ qui expose une conception résolument matérialiste de l'homme \cite{la_mettrie_homme_1748}.
Il y soutient que la pensée n'est pas le privilège d'une substance immatérielle mais une propriété de la \emph{matière organisée}.
...
\noindent
En 1859, Charles Darwin publie son ouvrage \an{On the origin of species by means of natural selection}  \cite{darwin_origin_1859}.
...
Cette vision incite à s'intéresser aux similitudes entre les processus de développement ontogénétique précoces chez les animaux et chez l'humain, en particulier ceux qui précèdent l'acquisition du langage. 
Elle invite à chercher comment mettre en place des prédispositions comportementales chez des robots analogues aux prédispositions innées que les êtres vivants héritent de leur lignée évolutive.
...
Cette idée est formulée dans sa \emph{maxime pragmatique} (1878) selon laquelle: 
\begin{quote}
...
Peirce détaille ce processus dans sa théorie de la \emph{sémiotique} basée sur trois catégories fondamentales: la \emph{priméité} (qualité pure), la \emph{secondéité} (fait réel, rencontre), et la \emph{tiercéité} (loi, médiation, habitude).
...
L'épistémologie pragmatique suggère qu'un agent artificiel devrait disposer de la capacité d'apprendre des habitudes par sédimentation hiérarchiques, en partant d'habitudes simples et courtes, puis en construisant des habitudes plus complexes en agrégeant des habitudes simples. 
...
Husserl insiste sur le rôle du \og dépôt \fg\ ou \og sédimentation d'habitudes \fg\ dans le processus de construction de sens.
Il ne nie pas l'existence du monde physique mais affirme qu'il est toujours révélé selon la manière dont la subjectivité le saisit et le construit dans la conscience.
%Il n’y a donc pas d’objet purement objectif, détaché de toute expérience vécue : le monde est toujours 
...
Dans les années 1970, une équipe du \an{Artificial Intelligence Lab} du MIT autour du roboticien Rodney Brooks et du philosophe Hubert Dreyfus a développé une approche de l'IA \og heideggerienne \fg\ donnant la place centrale aux comportements.
...
Il insiste sur un autre aspect de la philosophie heideggerienne qui n'a pas encore été prise en compte en IA, le fait que la fonction première de l'intelligence ne soit pas de résoudre des problèmes mais simplement de \og se débrouiller avec le monde \fg\ (\an{cope with the world}):
...
\noindent
La \an{Process Philosophy}, développée par Alfred North Whitehead dans son œuvre majeure \og \an{Process and Reality} \fg (1929), propose une nouvelle métaphysique appelée en français \emph{philosophie de l'organisme} ou \emph{philosophie du processus}. 
...
Il nomme \an{actual entity} un processus qui génère un flux d'événements, comme un organisme qui intègre des influences passées pour créer des expériences nouvelles. 
...
Cette théorie du processus offre une métaphysique dans laquelle les événements, le changement, la créativité et l'interrelation sont au centre de l'existence, transformant ainsi notre compréhension du monde depuis une perspective statique vers une dynamique de relations vécues et en devenir.

Considérer un robot comme un cas particulier d'\emph{entités actuelle} witeheadienne nous invite à nous focaliser sur les \emph{événements} liés à son existence: les événements internes liés à l'exécution de son \an{software}, autant que ceux liés à ses interactions sensorimotrices. 
Ces derniers peuvent être déclenchés par le robot, par exemple une collision avec un obstacle, ou par l'environnement, par exemple un bruit détecté par un micro.
Le potentiel de \emph{devenir} et de \emph{créativité} de cette entité actuelle réside dans sa capacité à reconnaitre et à générer des nouveaux types et motifs d'événements.   
...
Whitehead utilise l'exemple des organismes végétaux pour décrire les mécanismes de perception des entités actuelles qui ont une expérience de connexion au monde non consciente. 
Il fait la distinction entre deux modes de perception : le mode de l'\emph{immédiateté présentationnelle} (au sens de \og percevoir une couleur \fg) et le mode de la \emph{causalité efficiente} (au sens de \og percevoir un objet \fg). 
...
Les signaux provenant des capteurs sont reçus sur le mode de l'immédiateté présentationnelle.
Le robot apprend des régularités spatio-séquentielles d'action et de \an{feedback} sensoriels regroupés en événements d'interaction qui lui permettent de construire un modèle causal qui rend compte du mode de l'efficience causale de la perception.
Le software doit implémenter le processus qui permet à l'entité actuelle robotique d'apprendre à produire des événements perceptifs sur le mode de la causalité efficiente.
...
L'agent artificiel devra mettre en œuvre des boucles de production/falsification d'hypothèses efficaces pour répondre au problème exprimé par Wheeler.
...
Le terme \emph{génétique} doit s'entendre au sens de la genèse de la connaissance au cours du développement ontogénétique du sujet connaissant.
Vers la fin de sa vie, Piaget a rapproché l'épistémologie génétique de l'épistémologie constructiviste théorisée par d'autres auteurs comme Ernst von Glasersfeld \cite{von_glasersfeld_radical_1995}. 
L'épistémologie génétique élabore une théorie du mécanisme par lequel la connaissance peut être construite à partir de l'\emph{expérience vécue}, idée que l'épistémologie empiriste avait introduite avant elle :

%s'appuie sur l'épistémologie empiriste selon laquelle  le point de départ de toute connaissance est l'\emph{expérience vécue}, pour proposer une théorie du mécanisme de construction de connaissance :
...
Piaget part d'une hypothèse phénoménologique dans laquelle l'expérience d'interaction est centrale, et y intègre le rôle de l'habitude lié à l'épistémologie pragmatique.
Il explique l'acquisition d'habitudes d'expérience à partir d'unités élémentaires de l’activité mentale, qu'il appelle \emph{schèmes}. 
Un schème est une entité abstraite qui organise l'interaction du corps avec son environnement. 
Ces schèmes se renforcent lorsque l'expérience les conforte, ou se modifient lorsque qu'elle les contredit. 
Si l'épistémologie génétique est surtout connue dans le cadre du développement de l'enfant, elle continue à s'appliquer aux apprentissages qui accompagnent la pratique d'activités nouvelles tout au long de la vie.  
...
\begin{enumerate}%[itemsep=0pt, parsep=0pt, partopsep=0pt]
	\item Vécu singulier inscrit dans l'action, connaissance en actes.
	\item Vécu représenté, signifiants intériorisés, privés.
...
Les détracteurs de l'épistémologie génétique ont contesté le rôle fondamental que Piaget donnait à l'interaction sensorimotrice.
...
Des recherches récentes commencent à montrer l'importance des interactions à l'\oe uvre pendant l'embryogenèse animale, par exemple dans l'équipe de Michael Levin \cite{tung_embryos_2024}.
Même si Piaget semblait penser que le sujet se construisait par interaction avec le monde extérieur après la naissance, ses idées épistémologiques restent applicables aux interactions pré-natales. 
Quoi qu'il en soit, l'épistémologie génétique reste largement acceptée comme la référence fondatrice des théories cherchant à expliquer l'émergence de la connaissance à partir de la matière chimique et organique. 

Dans le cadre de l'intelligence artificielle développementale, l'épistémologie génétique apporte un angle d'attaque pour \og mécaniser \fg\ ou \og naturaliser \fg\ le processus de construction de connaissance chez un agent autonome.
Je retiens le concept central de \emph{schème sensorimoteur} comme construit cognitif qui organise l'expérience d'interaction entre le corps et l'environnement.
Un schème sensorimoteur opère comme un médiateur qui permet simultanément la construction d'une connaissance de soi et de l'environnement. 
%au sein du couplage corps/environnement.
...
Il conçoit l'individuation humaine dans la continuité de l'individuation animale: 
...
Sur cette base, Simondon explique l'individuation au sein d'un milieu \emph{pré-individuel} métastable à partir d'une forme ou d'une information initiale qui déclenche un processus de transduction. 
A la différence des solutions cristallines, cependant, les organismes vivants offrent un milieu au sein duquel la transduction engendre de nouveaux états métastables, susceptibles à leur tour d'accueillir de nouvelles transductions, en raison des interactions continues de l'organisme avec son environnement.
...
\noindent
Après avoir posé les bases de notre compréhension du processus de construction de connaissance par un \an{mind}, nous pouvons examiner les apports de la psychologie et les neurosciences sur le plan fonctionnel. 
Nous discutons tout d'abord de la façon dont la question de la conscience est sortie du cadre de la recherche en IA, pour ensuite envisager la cognition  comme un ensemble de processus de traitement de l’information produisant des comportements intelligents.
...
David Chalmers introduit la notion de \emph{zombi philosophique} pour désigner un agent qui se comporterait comme un être conscient tout en étant dépourvu de subjectivité. 
...
Cette erreur consiste à introduire un \emph{niveau intermédiaire} ou un \emph{module spécialisé} censé jouer le rôle de la conscience mais qui échoue à l'expliquer. 
...
Cette section vise à rappeler certains fondements issus de la psychologie cognitive sur les notions de représentation et de modèle mental qui sont parfois négligés dans les travaux d'intelligence artificielle. 
...
Ladrière \cite{ladriere_representation_1999} nous rappelle que le concept de représentation repose sur une double métaphore: celle de la \emph{représentation théâtrale} et celle de la \emph{représentation diplomatique}. 
...
L’univers subjectif, la vie mentale du sujet, lui apparaît à lui-même comme un fait qui existe et dont la notion de représentation mentale est une première tentative pour en rendre compte, une fois mis de côté le problème de la conscience. 
...
En revanche, il est possible d'effectuer des tâches automatiques, sans leur porter attention.
...
Cependant, même focalisée sur un objet externe, elle implique la capacité intérieure de pouvoir pointer une région de l'espace dans un modèle prédictif de façon à orienter le système sensorimoteur vers le point correspondant de l'environnement.
...
Il me semble important de retenir que les aspects de la corporéité interoceptive, émotionnelle, et sensorimotrice traversent tous les niveaux de ce découpage tripartite, du plus ancien au plus récent. 
Les sections suivantes examinent davantage les dimensions émotionnelles et spatio-temporelles de cet ancrage de l'activité cérébrale dans la corporéité. 
...
Chez l'homme, ils sont produits par des petits groupes de neurones situés dans la partie haute du tronc cérébral qui projettent leurs axones et diffusent ces neurotransmetteurs à travers tout le cortex. 
L'amygdale, située dans le télencéphale, joue un rôle crucial dans le contrôle de leur production et leur régulation.
...
Selon lui, les états émotionnels ont des effets généraux mais ne sont pas associés à des comportements spécifiques à chaque situation.
Ils présentent une dynamique suffisamment rapide pour permettre une adaptation efficace augmentant les chances de survie. 

La sérotonine renvoie à des aspects tels que la confiance en soi (\an{confidence}) et l'attitude posée.
Les quatre sommets du cube situés du côté de la sérotonine élevée correspondent à des situations propices à l'apprentissage et possiblement ludiques, y compris le sommet \an{Contempt/Disgust} (mépris, dégout) qui est à prendre dans le sens positif d'un refus calme et d'une capacité à dire \og non \fg. 
...
Les quatre sommets du cube situés du côté de la dopamine élevée correspondent à des situations marquantes liées à un renforcement de la mémoire d'expériences positives ou négatives.
Au contraire, les sommets du côté de la dopamine faible correspondent à des expériences fades, y compris la \emph{surprise} qui est à comprendre ici en tant qu'elle n'est encore pas qualifiée. 
...
Les sommets situés du côté de la noradrénaline élevée sont liés à un fort engagement: \an{anger/rage} peut être lié à des comportements de fuite ou de combat, alors que \an{Distress/Anguish} (détresse, angoisse) correspond à un état de fébrilité ou de tourment.
Ceux situés du côté de la noradrénaline faible sont liés à des états de sidération comme \an{Fear/Terror} ou de béatitude comme \an{Enjoyment/Joy}. 
...
Il ne s'agit pas de prétendre que le robot éprouverait des émotions ni même qu'il serait \og doté d'un système émotionnel \fg, mais plutôt d'examiner l'hypothèse que ce modèle permettrait de créer un robot qui mimerait les émotions, afin de permettre des apprentissages ouverts.
...
Il suggèrent que le cerveau s'appuierait sur ces capacités de calcul spatio-temporel héritées de l'évolution phylogénétique
%, et précoces sur le plan du développement ontogénique, 
pour naviguer dans des espaces conceptuels abstraits.  
...
Chaque niveaux du découpage cérébral tripartite présenté en section \ref{sec:anatomie} contient des capacités de calcul spatio-temporel.
En particulier, le télencéphale contient le complexe entorhinal-hippocampique qui entre en jeu dans l'organisation des activités spatio-temporelles complexes. 
Il contient un ensemble de neurones spatialement modulés, dont la découverte a été récompensée par le prix Nobel de médecine attribué en 2014 à John O'Keefe et Edvard Moser. 
...
Je reproduis l'une de leurs figures en Figure \ref{fig:space} pour donner un aperçu général de la complexité de ces mécanismes. 
...
Grieves et Jeffery énumèrent de nombreux types de neurones impliqués dans la représentation spatio-temporelles en soulignant qu'on peut encore s'attendre à d'autres découvertes.
...
La façon dont cette reconfiguration s'opère nous est encore largement inconnue sachant que les neurones ne reçoivent pas de représentation directe du monde. 
...
Ils mentionnent les \an{object cells} qui répondent à la présence d'un objet à un emplacement particulier, ou à l'absence d'un objet attendu à un endroit. 
Ils rapportent également des études investiguant des populations de neurones codant l'espace au delà de la région entorhinal-hippocampique. Certaines de ces études portent sur les \an{goal cells} dans le cortex préfrontal médian, 
...
Cette théorie propose que des centaines de milliers de modèles locaux spatio-temporels seraient construits au niveau du cortex pour mémoriser notre façon d'interagir avec chaque objet et chaque concept.
Ces modèles spatio-séquentiels locaux reposeraient sur un système de coordonnées universel permettant d'organiser toutes nos connaissances. 
%Selon lui, l’intelligence s'appuierait sur l’interaction de milliers de modèles locaux spatio-temporels capables d’apprendre la structure des objets et de l’environnement. Ces neurones serviraient de base à  un système de coordonnées universel permettant d’organiser différents types de connaissances. 

L'examen de ces mécanismes cérébraux me conduit à considérer les capacités de représentations spatio-temporelles comme un élément incontournable d'une architecture cognitive développementale bio-inspirée.
L'hypothèse de Buzsáki et Moser, interprétée dans le cadre de l'IA, suggère que de telles capacités constitueraient un socle sur lequel les capacités cognitives de plus haut niveau se développent. 
Le constat que ces capacités reposent sur des prédispositions innées du cerveau invite à programmer des prédispositions analogues chez un agent artificiel sous la forme de modules fonctionnels spécialisés.
Une telle approche fonctionnaliste entraine inévitablement un réductionnisme.
La question devient alors de savoir quel niveau de réductionnisme des capacités d'organisation spatio-temporelle des comportements serait le plus fécond pour le projet de l'intelligence artificielle développementale.

L'hypothèse des mille cerveaux de Hawkins peut être rapprochée de la théorie des schèmes de Piaget par le fait que le cerveau pourrait mémoriser des centaines de milliers de schèmes spatio-temporels pour représenter les interactions avec des objets concrets et manipuler des concepts abstraits. 
L'existence des \an{place cells} et des \an{grid cells} nous rappelle que deux représentations complémentaires sont utiles: une représentation sous forme de réseau capturant les transitions entre points, et une représentation topologique capturant les distances euclidiennes.  
...
Le récent livre de Stanslas Dehaene \og Le Rectangle de Lascaux: et homo sapiens inventa la géométrie \fg\ passe en revue de nombreuses études mettant en évidence les prédispositions du cerveau humain à manipuler des représentations géométriques et temporelles \cite{dehaene_rectangle_2026}.  
...
Elles rapportent deux systèmes géométriques distincts: le système fondamental de navigation (\an{core navigation system}) et le système fondamental d'analyse de formes (\an{core form analysis system}).  
...
Certaines études ont même tenté de déterminer ce qui comptait comme contour de l'environnement: des murs ou même des légères marches sur le sol \cite{cheng_purely_1986}. 
Inversement, des lignes ou des contours dessinés sur le sol n'étaient pas pris en compte comme contour ni des colonnes espacées.  
...
Ce système se base sur les relations relatives entre longueurs et sur les angles. Il se caractérise par une invariance d'échelle et une invariance de sens, c'est à dire qu'il est relativement insensible aux différences d'échelle et aux retournement en miroir.
Spelke souligne que les deux systèmes apparaissent donc pratiquement complémentaires: le système de navigation capture des distances absolues et le sens mais pas les longueurs relatives ni les angles, le système de reconnaissance de forme fait l'inverse. 
...
François Chollet \cite{chollet_measure_2019} a listé quatre savoirs fondamentaux que la plupart des agents  intelligents naturels auraient hérités de l'évolution phylogénétique : 
\begin{itemize}[label=\scalebox{.85}{$\diamond$}]
	\item Géométrie élémentaire, spatialisation: reconnaitre et manipuler des formes simples (lignes, rectangles, triangles, cercles). Différencier les formes même après des transformations (rotation, translation, symétrie, déformation). Détecter les positions et distances. 
...
Ces études sur le \an{core knowledge in the mind} élargissent notre réflexion engagée à la section précédente sur les mécanismes de représentation spatio-temporelles du cerveau. 
Elles suggèrent d'ajouter la capacité de prendre en compte l'existence d'objets, leurs propriétés physiques et leur éventuelle agentivité. 
La question de la répartition entre l'inné et l'acquis d'éventuels modules cognitifs liés à ces capacités est à nouveau posée. 
...
\noindent
De nombreux animaux manifestent des comportements propres à leur espèce essentiels à leur survie. 
Les exemples sont omniprésents : construction de toiles chez les araignées, poursuite d'objets en mouvements chez les chiens et les chats, ou encore migration chez diverses espèces volantes, aquatiques, ou terrestres. 
...
Blumberg précise que, bien que partiellement appris, les comportements instinctifs ne sont pas tous aussi malléables.
%la nature développementale des comportements instinctifs ne signifie pas que tous les comportements instinctifs soient également malléables. 
Au contraire, ils s'inscrivent dans un continuum allant des plus rigides aux plus flexibles. 
...
Les signaux olfactifs y jouent un rôle central, mais ce comportement implique déjà un apprentissage sensorimoteur combinant également des indices tactiles et parfois thermiques (e.g., \cite{hym_newborn_2021}, \cite{alberts_experience_2012}). 
%Dans des expériences chez l'humain, Charlotte Hym et ses coauteurs \cite{hym_newborn_2021} ont montré qu'ils impliquaient aussi des indices tactiles et parfois thermiques.  Jeffrey Alberts et April Ronca l'ont également observé chez d'autres mammifères \cite{alberts_experience_2012}. 
...
A ma connaissance, l'apprentissage développemental précoce de comportements instinctifs ne constitue pas, à ce jour, un domaine de recherche explicitement identifié en intelligence artificielle développementale.
Ce constat m'invite à proposer cette problématique comme une contribution originale de mon travail. 
Il s'agit d'identifier les interdépendances entre schèmes comportementaux innés et flexibilité comportementale, qui permettent à chaque agent de devenir une entité singulière au fil de son histoire propre, dans un processus d'individuation au sens de Simondon.
...
Dans son ouvrage \emph{Pour comprendre Jean Piaget} \cite{dolle_pour_1997}, Jean-Marie Dolle synthétise ainsi les stades chronologiques du développement cognitif de l'être humain :
...
\noindent
Les fondements théoriques passés en revue dans ce chapitre apportent chacun des sources d'inspiration complémentaires pour proposer une vision de l'intelligence artificielle développementale.

Dans cette vision, la connaissance est fondamentalement empiriste au sens où elle trouve ses origines dans l'expérience, ou, selon le terme de Hume, les impressions sensibles. 
Ces impressions sensibles peuvent être vues comme un flux d'événements ayant des valeurs affectives ou émotionnelles. 
La phénoménologie suggère que ce flux d'événements ne constitue pas un flux d'observation ou de perception d'une réalité présupposée:  l'agent n'observe pas directement une réalité déjà constituée d'objets prédéfinis.
Autrement dit, ces événements ne possèdent pas de contenu représentationnel en eux-même. 
La philosophie du processus de Whitehead suggère que les événements sont ontologiquement premiers.
Ils sont à l'origine d'un processus de différentiation a partir duquel s'organisent les interactions entre l'organisme et son environnement.
La \an{Theory of Event Coding} de Hommel apporte un éclairage complémentaire sur le rôle central des événements comme briques de base de la cognition.

Le processus de construction empiriste nécessite néanmoins certains présupposés: des préférences pour certains événements ainsi qu'une capacité à capturer leur enchainement et leur déploiement spatial.  
Ces présupposés peuvent être compris comme hérités d'un processus évolutionniste, par lequel chaque être vivant hérite de certaines préférences initiales, d'un certain potentiel de développement et d'une capacité à accéder à un certain niveau de connaissance.

L'épistémologie pragmatique conduit à relier la notion de flux d'événements à celles de comportement et d'habitude. 
L'agent influence le cours des événements sur la base de préférences et d'habitudes acquises.
En complément de la phénoménologie, le pragmatisme souligne le rôle central des habitudes dans l'attribution de signification, et suggère une \emph{inversion du raisonnement} selon laquelle le comportement précède la signification plutôt qu'il n'en découle.
Le processus d'acquisition d'habitudes peut lui-même obéir à une logique évolutionniste dans laquelle les habitudes favorables sont conservées et les habitudes défavorables oubliées.  


Les théories constructivistes invitent à considérer l'acquisition d'habitudes à travers un mécanisme d'organisation et de complexification progressive de \emph{schèmes}. 
Alors que l'agent fait l'expérience des événements avant de connaitre l'environnement, l'observateur extérieur, lui, voit ces événements comme des schèmes sensorimoteurs, c'est à dire des interactions avec cet environnement.
La psychologie développementale de Piaget propose une théorisation de ce mécanisme : la cognition se développe par la construction et l'organisation de schèmes de niveaux hiérarchiques croissants, chaque niveau d'organisation rendant possibles des formes d'interaction plus élaborées.
%La psychologie développementale de Piaget imagine les principes de fonctionnement de ce mécanisme de construction de schèmes de plus en plus haut niveau par lequel la cognition de l'agent se développe. 


Mon projet d'intelligence artificielle développementale consiste à traduire sous forme de simulations et d'expérimentations informatique cette théorie de l'intelligence qui part des événements et conduit à une organisation cognitive progressive. 
Il vise à en réaliser des démonstrations, en tester les limites, et en estimer les possibilités d'application.


%Cette inversion du raisonnement qui donne la primauté aux événements et à l'organisation cognitive des comportements offre prise à la simulation et à l'expérimentation informatique pour en faire la démonstration et en tester les limites. 

%Cette conception du développement cognitif offre prise à des simulations et expérimentations informatiques pour en faire la démonstration et en tester les limites. 




%Il sont au contraire à l'origine d'un processus de différentiation par lequel le sujet découvre le monde en se construisant lui-même.
...
%Construire et réitérer des comportements spatio-séquentiels hiérarchiques basés sur des préférences intéractionnelles initiales La possibilité de réitérer des comportements appris à un haut niveau hiérarchique de manière adaptée implique la capacité de construire des représentations pertinentes du monde tel que l'agent en fait l'expérience. 
