...
Cependant, même focalisée sur un objet externe, elle implique la capacité intérieure de pouvoir éliciter une région de l'espace dans un modèle prédictif de façon à orienter le système sensorimoteur vers le point correspondant de l'environnement.
...
Chez l'homme, il sont produits par des petits groupes de neurones situés dans la partie haute du tronc cérébral qui projettent leurs axones et diffusent ces neurotransmetteurs à travers tout le cortex. 
...
Il ne s'agit par de prétendre que le robot éprouverait des émotions ni même qu'il serait \og doté d'un système émotionnel \fg, mais plutot d'examiner l'hypothèse que ce modèle permettrait de créer un robot qui mimerait les émotions et permettre des apprentissages ouverts.
...
La question devient alors de savoir quel niveau de réductionnisme des capacités d'organisation spatio-temporelle des comportements serait le plus fécond dans pour le projet de l'intelligence artificiel développementale.
...
Spelke souligne que les deux systèmes apparaissent donc pratiquement complémentaire: le système de navigation capture des distances absolues et le sens mais pas les longueurs relatives ni les angles, le système de reconnaissance de forme fait l'inverse. 
...
\begin{itemize}[label=\scalebox{.85}{$\diamond$}]
	\item Géométrie élémentaire, spatialisation: reconnaitre et manipuler des formes simples (lignes, rectangles, triangules, cercles). Différencier les formes même après des transformations (rotation, translation, symétrie, déformation). Détecter les positions et distances. 
...
Les exemples sont multiples: construction de toiles chez les araignées, poursuite d'objets en mouvements chez les chien et les chats, ou encore migration chez diverses espèces terrestres, volantes, ou aquatiques. 
