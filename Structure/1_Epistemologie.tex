...
Ce chapitre vise à définir l'intelligence artificielle précoce telle qu'elle est mise en \oe uvre par les êtres vivants, sur la base de l'épistémologie, la psychologie et les sciences cognitives. 
...
Il y soutient que la pensée n'est pas le privilège d'une substance immatérielle mais une propriété de la matière organisée, affirmant ainsi que \emph{la matière peut penser}.
Il formule de manière provocatrice cette thèse moniste: \og Concluons hardiment que l'homme est une machine et qu'il n'y a dans tout l'Univers qu'une seule substance \fg.
...
Pour Ey \cite{ey_conscience_1999}, \og dire d'un être qu'il sent, qu'il perçoit, qu'il se souvient de quelque chose, qu'il prépare une action ou qu'il se sent ou se sait être quelqu'un qui dirige son existence vers telle ou telle fin, c'est toujours et nécessairement dire qu'il est conscient \fg.
La conscience ne semble pourtant pas admettre, à l’heure actuelle, de définition ni de théorie faisant consensus au sein de la communauté scientifique. 
...
Malgré la nature mystérieuse de la conscience, il existe une littérature consacrée la conscience artificielle comme en témoigne la création en 2009 du \an{International Journal of Machine Consciousness}, renommé en 2020 \an{Journal of Artificial Intelligence and Consciousness}. 
Plus récemment, Joscha Bach a contribué à fonder le \an{Californian Institute for Machine Consciousness} (CIMC), en défendant l'idée que la recherche sur la conscience des machines constitue un enjeu scientifique majeur :
...
Le philosophe David Chalmers a proposé une distinction célèbre entre deux niveaux de difficulté : \an{the easy problem of consciousness} et \an{the hard problem of consciousness} \cite{chalmers_conscious_1997}.
Le \an{easy problem} consiste à expliquer comment un système peut produire des comportements  \og comme s'il était conscient \fg. 
Chalmers qualifie ce problème de facile non pas parce qu'il estime sa résolution imminente,  mais parce qu'il estime qu'aucun obstacle conceptuel fondamental ne s'y oppose. 
L'étude de ce problème pourrait impliquer la construction de robots qui mimeraient la conscience sans la posséder---Chalmers parle de \emph{zombies philosophiques}. 
...
Cette erreur consiste à introduire un \emph{niveau intermédiaire} ou un \emph{module spécialisé} censé rendre compte de la conscience mais qui échouent à l'expliquer. 
...
Mon travail s’inscrit en affinité avec les positions de Manzotti et Chella, en privilégiant une approche centrée sur les interactions.
Sans prétendre alimenter directement une théorie de la conscience, l'approche interactionaliste rejoint des questions de  \emph{motivation intrinsèque} et de \emph{construction de sens}. 

\subsection{Représentation, mémoire, modèle du monde}

\og Etre conscient c'est disposer d'un modèle personnel de son monde. \fg 
Cette citation de Ey \cite{ey_conscience_1999} rattache d'emblée la notion de conscience à la question des représentations mentales. 
...
Ladrière \cite{ladriere_representation_nodate}présente le concept de représentation comme reposant sur une double métaphore, celle de la représentation théâtrale et celle de la représentation diplomatique. 
...
C’est en référence à cette conception philosophique de la notion de représentation mentale que nous tentons d’expliquer l’activité, en recourant à l’étude plus approfondie qu’en fait la psychologie cognitive. 
...
Cet article reprend le terme \emph{attention} à des modèles antérieurs utilisés en traduction automatique, et introduit la technologie des \an{transformers} qui ouvre la voie aux grands modèles de langage actuels.
Dans ce contexte, ce terme désigne un mécanisme computationnel permettant d'attribuer des poids de saillance différenciés aux éléments d'une séquence en fonction d'un contexte donné. 
...
L’attention peut par ailleurs être dirigée vers des objets externes de l’environnement ou vers des contenus internes, tels que des pensées ou des représentations mentales. 
...
Certaines tâches et activités sont apprises sous controle attentionnel puis deviennent automatiques.  
...
Cette citation fait écho à la \an{Feature Integration Theory} \cite{treisman_feature-integration_1980}, selon laquelle les caractéristiques élémentaires des objets (couleur, orientation)  sont traitées en parallèle, tandis que leur intégration en une représentation cohérente d’objet requiert une mobilisation attentionnelle. 
...
\noindent
Cette section a pour objectif de rappeler modestement quelques éléments fondamentaux concernant le cerveau en tant qu’organe biologique dont la fonction première est l’organisation et le contrôle des comportements dans le temps et l’espace, bien avant l’émergence de capacités de représentation abstraite et de raisonnement symbolique.
...
Ce découpage tripartite est également associé à trois stades successifs de l’évolution phylogénétique survenus approximativement dans cet ordre. 
...
Il me semble important de retenir qu'il y a deux dimensions de l'activité cérébrale qui se projettent à tous les niveaux de ce découpage tripartite, du plus ancien aux plus récent: la dimension affective et la dimension spatio-temporelle.
...
Il explique que ces neurotransmetteurs sont très anciens dans l'échelle phylogénétique et sont présents chez des animaux allant des nématodes au vertébrés. 
...
Il ne considère pas que les états émotionnels soient associé à des comportements spécifiques à chaque situation mais au contraire qu'ils ont un effet général tout en présentant une dynamique suffisamment rapide pour permettre une adaptation efficace aux changements de l’environnement. 
...
Talanov montre que l’intégration du modèle émotionnel de Lövheim permet de concevoir des robots dont les actions ne sont pas uniquement dictées par des objectifs extrinsèques ou des règles prédéfinies, mais émergent également de dynamiques motivationnelles internes. 
...
\subsection{Apprentissage spatio-séquentiel}

%Plusieurs travaux en neurosciences cognitives suggèrent que les mécanismes neuronaux impliqués dans la navigation spatiale constituent un socle computationnel général pour des formes d’intelligence plus abstraites. 
\noindent
György Buzsáki et Edvard Moser ont avancé l’hypothèse selon laquelle les mécanismes neuronaux sous-jacents à la navigation dans l’espace physique et l'espace mental reposeraient sur des principes communs.
Les représentations spatio-temporelles pourraient ainsi servir de fondement à la cognition abstraite. 
Il suggèrent que le cerveau pourrait s'appuyer sur des capacités de calcul spatio-temporel héritées de l'évolution phylogénétique, et précoces sur le plan du développement ontogénique, pour naviguer dans des espaces conceptuels abstraits.  
...
Comme évoqué à la section \ref{sec:anatomie}, des capacités de calcul spatio-temporelles sont présentes à tous les niveau du découpage tripartite du cerveau. 
En ce qui concerne le télencéphale, le complexe entorhinal-hippocampal semble jouer un role clé grace à un ensemble de neurones spatialement modulés, dont la découverte a conduit au prix Nobel de médecine en 2014 de John O'Keefe et Moser. 

Par exemple, Hideyoshi Igata et ses coauteurs ont 
montré que des neurones de l'hippocampe pouvaient rejouer des séquences en ordre inversé afin de reconstruire les positions ayant conduit à une récompense. Ils ont observé que l’hippocampe a même pu rejouer des trajectoires optimisées qui n’ont pas été directement explorées par l’animal \cite{igata_prioritized_2021}. 
%Ces résultats suggèrent un rôle central du rejeu hippocampique dans la planification et l’optimisation comportementale.
Stéphanie Theves a montré que l'hippocampe codait des distances pertinentes dans un espace conceptuels définis par des caractéristiques d'objets, suggérant l'existence d'une cartographie cognitive des concepts  \cite{theves_hippocampus_2019}. 
Eric Knudsen a montré que des neurones hippocampiques chez le primate peuvent former des \og gradients de valeur \fg le long d’un continuum abstrait de récompense, constituant une carte cognitive d’un espace de valeur motivationnelle \cite{knudsen_hippocampal_2021}.

%L’hippocampe participe également à l’apprentissage séquentiel par la réactivation de trajectoires spatio-temporelles. 

La plupart des auteurs insistent sur l'étendue des choses qu'ils nous restent encore à comprendre de la façon dont le cerveau gère l'espace et le temps. 
Dans leur article \og \an{The representation of space in the brain} \fg, Roddy Grieves et Kate Jeffery tentent une synthèse des différentes études sur ce sujet \cite{grieves_representation_2017}. 
Je reproduit l'une de leurs figures en Figure \ref{fig:space} non pour entrer dans ce niveau de détail mais pour illustrer la complexité des ces mécanismes. 
...
Grieves et Jeffery listent différents ensembles neuronaux décris dans la litérature, en soulignant la rapide évolution de ce champ de recherche. 
Les \an{place cells} s’activent lorsque l’animal occupe une position spécifique dans un environnement donné, indépendamment de son orientation. 
...
La population de \emph{place cells} peut être reconfigurée (\emph{remapping}) pour correspondre à un environnement donné, contribuant ainsi à la navigation dans cet environnement et à la mémoire épisodique.

Les \an{head-direction cells} encodent l’orientation de la tête par rapport à un repère allocentrique. Leur activité dépend à la fois d’indices sensoriels et de mécanismes d’intégration du mouvement. 
%Des expérience chez le rat on observé l'origine de ce signal de direction dans le tronc cérébral. 
%tandis que chez l’humain il est observé notamment dans le thalamus et le cortex entorhinal.
...
Les auteurs soulignent que l'hippocampe est connu pour être impliqué dans des processus qui vont bien au delà de la navigation spatiale, incluant la discrimination d'objets. 
Il signalent les \an{object cells} qui répondent à la présence d'un objet à des emplacements particuliers, ou alors peuvent répondre à des objets manquant qui ont étés retirés de l'environnement. 
%Ces différentes représentations suggèrent que l’hippocampe intègre des informations spatiales et non spatiales, notamment liées aux objets, participant ainsi à la formation de la mémoire épisodique.

Au delà de la région entorhinal-hippocampal, Grieves et Jeffery rapportent des études investiguant la présence de \an{goal cells} dans le cortex préfrontal médian. 
Par ailleurs, de nombreuses cellules codent l’espace en relation avec l’action. Dans le striatum, par exemple, certains neurones s’activent préférentiellement lorsqu’un mouvement spécifique est exécuté à un endroit particulier, reflétant le rôle de cette structure dans l’apprentissage d’actions, la formation d’habitudes et l’automatisation comportementale.
Des \an{conjunctive cells} encodent simultanément des caractéristiques spatiales et comportementales. 
Des neurones sensibles aux séquences d’actions ont également été observés dans le cortex pariétal supérieur et dans le cortex rétrosplénial.

%Certaines études décrivent également des \emph{goal cells}, notamment dans le cortex préfrontal médian, dont l’activité augmente lorsque l’animal s’approche d’un objectif de navigation, indépendamment de la récompense associée. 

%Enfin, la dimension temporelle joue un rôle essentiel dans ces mécanismes : les rythmes neuronaux et la dynamique séquentielle structurent l’encodage et la consolidation des représentations spatio-cognitives.

%\paragraph{Implications pour l’intelligence artificielle}
...
Ce type de neurones constituerait un système de coordonnées universel permettant d’organiser différents types de connaissances. 
...
Des auteurs comme Buzsáki et Moser ont émis l'hypothèse que  \og \an{the neuronal algorithms underlying navigation in real and mental space are fundamentally the same} \fg, suggérant que les fonctionnalités spatio temporelles pavent le chemin vers l'intelligence abstraite.  \cite[p.~130]{buzsaki_memory_2013}
Le cerveau humain recyclerait ainsi les capacités de calcul spatial héritées de l'évolution phylogénétique pour traiter des concepts abstraits. 
Dans un espace conceptuel construit à partir de dimensions de caractéristiques d’objets, l’hippocampe code les distances dans l’espace conceptuel pertinent, et non simplement dans l’espace des caractéristiques physiques, montrant qu’il mappe un espace de concepts plutôt qu’un simple espace de traits \cite{theves_hippocampus_2019}.


Des neurones hippocampiques chez le primate forment des « champs de place de valeur » le long d’un continuum abstrait de valeur de récompense, créant une carte d’un espace de valeur purement cognitif \cite{knudsen_hippocampal_2021}

Hideyoshi Igata et ses collègues ont montré que l'hippocampe rejouait des séquences spatio-temporelles en ordre inversé pour reconstituer les positions ayant conduit à une récompense. 
Pendant une phase d'apprentissage les neurones de l'hippocampe ont même rejoué un chemin optimisé qui n'avait pas été exploité par l'animal. 
\cite{igata_prioritized_2021}
...
On observe des prouesses de navigation étonnantes de la part de nombreux animaux. 
La confirmation de la nature spatiale du complexe entorhinal-hippocampal a conduit au prix nobel de médecine de John O'Keefe et Edvard Moser en 2014.
Les place cells, head-direction cells et grid cells forment semblent etre les trois plus importantes d'un système que nous commençons seulement à comprendre. 
Les place cells sont situées dans l'hippocampe. Elles forment un réseau de places qui sont reconnues par des indices sensoriels et moteurs et qui s'activent quand l'animal est situé à cet endroit indépendamment de sa direction.
Place fields represent higher order constructs assembled from more primitive spatial ones such as direction, boundaries, and self-motion information
La population des place cells peux etre remapped pour correspondre à un environnement donner et permettre la navigation et la mémoire dans cet environnement. 

Les head direction cells encode la direction de la tête par rapport à une direction choisie comme référence dans un environnement donnée. Elles sont informées par de indices sensoriels et par intégration des déplacements. 
Chez le rat on pense que le signal de direction de la tête trouve son origine dans le haut du tronc cérébral et est projeté dans d'autres régions du cerveau. 
Chez l'humain on trouve ce signal dans différentes régions incluant le cortex entorhinal et le thalamus. 

Les grid cells, situées dans le cortex entorhinal forment un pavage triangulaire.    
Certains chercheurs ont suggérés qu'elles étaient impliquées dans le calcul des déplacements par path integration. 

Mais les place cells, head direction cells, et grid cells ne sont que le début de ce qui semble une large game de neurones spatialement modulés. 
Les auteurs insistent aussi sur l'importance des rythmes temporels. Ils notent que les cellules qui sont modulées par la vitesse de déplacement sont communes à travers tout le cerveau.  
On a aussi découvert des speed cells qui encodent la vélocité de l'animal.  

Les boundary cells répondent uniquement aux bordures de l'environnement. 
Elles ont une relation complexe avec les place cells et les grid cells qui n'est pas encore bien comprise . 
Certaines de ces cellules ont une direction de déclenchement favorite: Elle se déclenchent quand l'anamial rencontre une bordure dans une direction allocentrique donnée. 

object cells. 
Ces cellules répondent à  la présence d'un objet à un emplacement particulier de l'environnement. 
Certaine cellules répondent à la l'emplacement d'un objet manquant qui a été retiré de l'environnement. 

The hippocampus is known to be involved in processes far beyond purely spatial navigation (Eichenbaum et al., 1999) including the discrimination of non-spatial cues such as objects (Clark et al., 2000; Fortin et al., 2004). For instance place cells in the hippocampus have been shown to respond to the spatial location of objects (LenckSantini, Rivard, Muller and Poucet, 2005) so this information is certainly being utilised by the hippocampus and thus possibly also in the formation of episodic memory.

Goal cells 
La recherche de goal cells n'en est encore qu'a ses débuts. 
Hock et al 2005 ont proposé des cellules situées dans le cortex préfrontal médian chez le rat qui s'active quand l'animal approche l'objectif de navigation indépendement d'une récompense fournie à cet endroit. 
Il a été proposé que le déclenchement des grid cells pourrait se traduire par une estimation de la distance à un objectif spatial (Stemmler et al 2015)

conjunctive cells encode simultanément des caractéristiques spatiales, environnementales et comportementales. 
Certaines grid celles répondent à une conjonction de direction, position, et vitesse. 

Movement or action-sensitive cells. 
de nombreuses cellules encodent l'espace en relation avec le mouvement.
Certaines cellules déclenchent autant quand le mouvement est actif que passif. 
Certaines cellules du striatum (partie des basal ganglia dans le télencéphale) 
Le striatum est impliqué dans l'apprentissage d'actions par rapport à des taches et la formation de réponses automatiques et habitudes.  
Certaines cellules de striatum déclenchent préférentiellement quand un anmial fait un mouvement particulier à un emplacement particulier d'un environnement (Thorn et al 2010).
De manière similaire, des neurones qui correspondent à des séquences de réponses comportementales ont été trouvées dans le cortex parietal supérieur
Certains neurones du cortexe retrospinal encodent des séquences de réponse et des positions spatiales. 
...
Jeff Hawkins propose une approche de l'intelligence artificielle à travers sa théorie des mille cerveaux (\an{Thousand Brains Theory}), qui s'appuie sur la découverte que des structures neuronales similaires aux cellules de grille (grid cells) sont distribuées dans l'ensemble du néocortex.
mais qu'elles sont présentes dans toutes les colonnes corticales. Elles fournissent un système universel de coordonnées pour organiser toute forme de connaissance.
Des milliers de modèles locaux : Plutôt qu'un modèle unique et centralisé, le cerveau utiliserait des milliers de petits modèles spatio-temporels locaux pour apprendre et interagir avec différentes configurations de l'environnement et différents types d'objets 
\cite{hawkins_framework_2019}
...
Legg et Hutter \cite{legg_universal_2007} proposent une synthése des principales techniques de mesure de l'intelligence naturelle afin d'en dégager des pistes pour établir des \an{benchmark} d'évaluation de l'intelligence artificielle. 
...
Ils visent à mesurer la capacité de l'agent à mettre en oeuvre une stratégie qui peut donner lieu à un apprentissage au cours du déroulé du test. 
...
Les tests \emph{statiques} s'appliquent principalement aux systèmes d'apprentissage supervisés et aux grands modèles d'IA générative. 
...
Il présente l'inconvénient de nécessiter un jeu de questions-réponses fourni par des experts et inconnu du modèle, ce qui implique de le renouveler constemment.
Un autre inconvénient est qu'il ne distingue pas clairement ce qui relève de l'intelligence ‘‘pure'' et ce qui dépend de connaissances préalables, notamment en mathématiques ou en physique.

Pour éviter d'impliquer des savoirs scolaires et éducatifs, François Chollet propose le \an{Abstract Reasoning Challenge} (ARC), un benchmark qui n'utiliser qu'un \emph{socle de connaissance} essentiel de la cognition naturelle \cite{chollet_measure_2019}.
Ce socle de connaissance (\an{core knowledge}) comprend quatre savoirs fondamentaux que la plupart des agents  intelligent naturels auraient hérités de l'évolution phylogénétique : 
...
L'ARC Challenge consitute un second exemple intéressant du quadrant \emph{statique} / \emph{objectif}. 
Il s’inspire de la structure des tests psychotechniques et propose des problèmes de raisonnement abstrait fondés sur la détection de motifs et de transformations logiques. 
Il évalue ainsi la capacité d’un système à généraliser à partir d’exemples et à inférer des règles.
La Fig. \ref{fig:ARC} montre un exemple mettant en jeu le \an{core knowledge} d'agentivité.
...
Ce benchmark évalue les LLMs en organisant des confrontations en aveugle opposant les modèles deux à deux.
Il produit un classement de type Elo sur la base d'un jugement humain ‘‘crowdsourcé'', les utilisateurs proposant leurs propres prompts et sélectionnant la meilleure réponse sans connaitre les modèles en compétition. 

\subsection{Tests dynamiques}

Les tests \emph{dynamiques} concernent principalement les systèmes d'apprentissage non supervisés ainsi que les agents et robots autonomes. 
...
Les jeux de plateau offrent des benchmarks emblématiques de ce quadrant. 
...
L'idée du test de Turing illustre l’esprit de ce quadrant, mais n'a pas été mise en oeuvre de manière convaincante. 
...
Alexander Maye et Andreas Engel démontrent par cette méthode que leur robot acquièrt une maitrise des ‘‘contingences sensorimotrice'' (voir Section \ref{sec:SMC}).
...
L’analyse comportementale illustrée en Figure \ref{fig:maye} met en évidence les contingences sensorimotrices propres à un couplage robot-objet particulier, représentées sous la forme d’une séquence d’actions et de signaux sensoriels.
...
Certaines études considèrent la construction de son propre modèle du monde comme un objectif en soit pour l'agent.
Je classe les benchmarks évaluant cette capacités dans la catégorie \emph{jugement} car ils ne mesurent pas directement l'aptitude d'un agent à accomplir une tâche appliquée: c'est l'expérimentateur qui juge la qualité du modèle du monde appris.
Mais ce jugement se fait souvent en présupposant le format de représentation utilisé par l'agent, ce qui complique la comparaison entre agents utilisant des formats de représentation différents.

Pour comparer la construction de modèles du monde par des agents utilisant des formalismes différents, certains auteurs adoptent une approche hybride. 
Dans leur article \an{Benchmarking world-model learning} \cite{warrier_benchmarking_2025}, Archana Warrier et al. proposent \emph{WorldTest}, un benchmark destiné à évaluer l'apprentissage non supervisé d'un modèle du monde en soumettant l'agent à des tâches avec un but objectif.
...
Il permet d'évaluer la capacité de l'agent à généraliser à partir de l'environnement de base, en mesurant ses performance à atteindre l'objectif défini dans l'environnement dérivé, de manière indépendante du formilsme de représentation utilisé.
...
Mes travaux se situent principalement dans le quadrant supérieur droit de la Figure \ref{fig:mesures}: les mesures dynamiques faisant appel au jugement humain, soit par interaction directe d'un humain avec l'agent, soit par analyse des comportements basés sur des enregistrements ou des traces. 
