...
Legg et Hutter introduisent ensuite une second distinction, cette fois entre les tests fondés sur une \emph{mesure objective} ceux fondés sur un \emph{jugement humain}.
Les premiers mesurent la capacité de l'agent a produire des réponses attendues ou à atteindre des objectifs préalablement définis.
...
Alexei Samsonovich, citant John von Neumann:  \og \an{If you will tell me precisely what it is that a machine cannot do, then I can always make a machine that will do just that} \fg , soutient que la maximalisation d'une mesure objective peut conduire au développement d'algorithmes qui ne méritent pas le qualificatif \emph{intelligent} \fg \cite[p.~105]{samsonovich_roadmap_2012}.
Un système varitablement intelligent doit être capable de faire quelque chose pour lequel il n'a pas été spécifiquement programmé.
Le célèbre test de Turing s’inscrit précisément dans cette catégorie de tests, puisqu’il demande à des observateurs humains de juger la capacité d’une intelligence artificielle à imiter une intelligence naturelle.
...
\begin{figure}[ht]
	...
	axe \emph{statique} vs \emph{dynamique} selon que la mesure repose sur des réponses immédiates ou sur une période d'interaction; 
...
Les tests statiques concernent surtout les systèmes d'apprentissage suppervisés et les grands modèles d'IA générative. 
Un exemple de \an{benchmark} appartenant au quadrant \emph{statique} / \emph{objectif} de la Figure \ref{fig:mesures} est le \an{Measuring Massive Multitask Language Understanding} (MMLU) \cite{hendrycks_measuring_2021}. 
Il s’agit d’un ensemble très large d’évaluations couvrant de nombreuses disciplines académiques. 
Il mesure la solidité générale du raisonnement et la capacité du modèle à transférer ses connaissances sur des domaines multiples.
Un inconvénient majeur est qu'il nécessite un jeu de question/réponse fourni par des experts et initialement inconnues du modèle.
