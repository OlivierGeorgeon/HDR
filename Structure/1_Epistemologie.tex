...
Dans leur article \og \an{Universal Intelligence: A Definition of Machine Intelligence} \fg, Shane Legg et Marcu Hutter note que \og \an{A fundamental problem in artificial intelligence is that nobody really knows what intelligence is} \fg \cite[p.~1]{legg_universal_2007}.

Si nous ne savons pas vraiment ce qu'est l'intelligence, nous ne partons quand meme pas de zéro. 
Cette question a occupé des nombreux penseur depuis de nombreux siècle et je crois qu'il est important de savoir nous y référer. 
...
\noindent
Je ne peux pas terminer cette vue d'ensemble de la notion d'intelligence sans examiner les différentes méthodes utilisées pour la tester et la mesurer.
Legg et Hutter \cite{legg_universal_2007} synthétisent les techniques de mesure de l'intelligence naturelle bien connue des experts pour un tirer des propositions de formalisation de \an{benchmark} d'évaluation de l'intelligence artificielle. 

Ils introduisent une première distinction utile qui différencie les \emph{tests statiques} des \emph{tests dynamiques}. 
Les tests statiques mesurent la capacité à résoudre une problème présenté en une seule fois. 
Ils se focalisent sur l'exploitation de connaissance et la mise en \oe uvre d'un raisonnement pour réponse à une question posée. 
%Ils ne mesurent pas directement la capacité à apprendre et à s'adapter au cours du temps. 
Au contraire, les tests dynamique se déroule pendant une certaine période de temps au cour de laquelle l'agent interagit avec l'environnement de test. 
Ces tests permettent de mesure la capacité de l'agent à mettre en oeuvre une stratégie qui peut donner lieu à un apprentissage pendant la phase de test. 

Legg et Hutter font ensuite une second distinction entre les tests par \emph{mesure objective} et les test impliquant un \emph{jugement humain}.
Les tests par mesure objective mesurent la capacité de l'agent a fournir des réponses attendues ou à atteindre des objectifs prédéfinis.
Dans ce cas, les auteurs imaginent une \og mesure universelle de l'intelligence \fg qui serait la somme infinie des score obtenue normalisés dans tous les environnement possibles pondérée par leur complexité. 
Ils notent qu'une telle mesure n'est pas en principe calculable mais qu'elle fournit une représentation mentale de ce qu'ils appellent une \an{mesure of universal intelligence}. 
...
Certains auteurs ont défendu la nécessité du jugement humain pour l'évaluation de l'intelligence. 
Par exemple, Alexei Samsonovich cite John von Neumann:  \og \an{If you will tell me precisely what it is that a machine cannot do, then I can always make a machine that will do just that} pour argumenter que la maximalisation d'une mesure objective conduit au développement d'algorithmes qui ne méritent pas le qualificatif \emph{intelligent} \fg \cite[p.~105]{samsonovich_roadmap_2012}.
Un système vraiment intelligent doit être capable de faire quelque chose pour lequel il n'a pas été spécifiquement programmé.
Le célèbre test de Turing rentre dans cette catégorie car il repose sur le jugement d'observateurs humains pour évaluer la capacité d'une intelligence artificielle à imiter une intelligence naturelle. 

Les deux axes de distinction proposés par Legg et Hutter délimites les quatre quadrants représentés en Figure \ref{fig:mesures}. 
...
\begin{figure}[ht]
	...
	axe \emph{statique} vs \emph{dynamique} selon que l'intelligence soit mesurée par des réponses immédiates ou au cours d'une période d'interaction; 
	axe \emph{mesure objective} vs \emph{jugement} selon que la mesure soit formalisée ou proviennent d'un acte de jugement humain.}
	\label{fig:mesures}
\end{figure}


La quadrant \emph{Statique} \ \emph{objectif} de la Figure \ref{fig:mesures} compte comme exemples typiques les méthode d'évaluation des grands modèle génératifs. 
Par exemple le \an{Measuring Massive Multitask Language Understanding} (MMLU) \cite{hendrycks_measuring_2021} est un benchmark célèbre pour l'évaluation des LLMs.
Un inconvénient majeur de cette approche est qu'elle nécessite un jeu de questions avec les réponses correctes attendues qui doit être tenu secret comme pour des examens. 
De plus il présuppose des connaissances préalables comme des connaissances de mathématique ou de physique. 

Le \an{Abstract Reasoning Challenge} a été développé pour utiliser un minimum de connaissance préalable et pour évaluer la capacité à utiliser uniquement un socle de connaissance que tout enfant ou agent naturellement intelligent est censé posséder pour résoudre des taches nouvelles, sans faire appel à des savoirs scolaires, linguistiques, ou culturels. 
...
Dans son article ``\an{On the measure of intelligence}'', Francois Chollet \cite{chollet_measure_2019} identifie quatre \an{core knowledge} : 
La Fig. \ref{fig:ARC} montre un exemple de problème mettant en jeu le \an{core knowledge} d'agentivité.
...
\subsection{Mesures de l'apprentissage d'un modèle du monde}

Dans leur article \an{Benchmarking world-model learning} \cite{warrier_benchmarking_2025}, Warrier et al. identifient quatre catégories de \an{benchmarks} destinés à évaluer l'apprentissage d'un modèle du monde:

\begin{itemize}[label=\scalebox{.85}{$\diamond$}]
	\item les \an{non-interactive benchmarks} évaluent la capacité de l'agent à inférer des règles sous-jacentes à partir d'exemples et à les généraliser à de nouveaux cas. Un exemple typique est le ARC challenge présenté en Section~\ref{sec:arc}
	...
	Elle empêche la comparaison entre différents formats de représentation.  
	\item les \an{benchmarks} avec récompense objective utilisés en apprentissage par renforcement tels qu'\an{Open Gymnasium} de OpenAI.  
...
Warrier propose WorldTest, une évolution des \an{unsupervised RL benchmarks}, qui différencie un \og environnement de base \fg durant la phase d'interaction, et un \og environnement dérivé à objectif explicite \fg durant la phase de test.
Ce dernier diffère de l'environnement de base tout en lui restant corrélé. 
WorldTest évalue la capacité de l'agent à généraliser à partir de l'environnement de base, en mesurant ses performance à atteindre l'objectif défini dans l'environnement dérivé. 
...
Mes travaux se situent principalement dans le quadrant supérieur droit de la Figure \ref{fig:mesures}: les mesures dynamiques faisant appelle au jugement humain, soit par interaction directe d'un humain avec l'agent, soit par analyse des comportements basés sur des enregistrements ou des traces. 
...
Chatbot Arena \cite{chiang_chatbot_2024}.
