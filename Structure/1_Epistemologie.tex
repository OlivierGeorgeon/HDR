...
Les tests statiques s'appliquent surtout aux systèmes d'apprentissage supervisés et aux grands modèles d'IA générative. 
Un premier exemple situé dans le quadrant \emph{statique} / \emph{objectif} est le \an{Measuring Massive Multitask Language Understanding} (MMLU) \cite{hendrycks_measuring_2021}. 
Il regroupe un ensemble très large d’évaluations couvrant de nombreuses disciplines académiques pour mesurer la solidité générale du raisonnement et la capacité à transférer les connaissances d'un domaines à l'autre.
Un inconvénient majeur est qu'il nécessite un jeu de questions/réponses fourni par des experts et initialement inconnues du modèle, ce qui implique de le renouveler constemment.
De plus, il présuppose des connaissances préalables, telles que des connaissances de mathématique ou de physique, ce qui floute la distinction entre connaissance et intelligence. 

Pour éviter d'impliquer des savoirs scolaires et éducatifs, François Chollet propose de n'utiliser qu'un \emph{socle de connaissance} (\an{core knowledge}) fondamental de la cognition naturelle \cite{chollet_measure_2019}.
Il formule l'hypothèse que tout agent naturellement intelligent posséde quatre savoirs hérités de l'évolution phylogénétique: 
...
Sur cette base, Chollet propose le \an{Abstract Reasoning Challenge} (ARC) qui consitute un second exemple du quadrant \emph{statique} / \emph{objectif}. 
Ce \an{benchmark} s’inspire de la structure des tests psychotechniques, en proposant des problèmes de raisonnement abstrait fondés sur la détection de motifs et de transformations logiques. 
