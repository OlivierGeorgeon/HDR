...
Il existe un certaine littérature sur la conscience machine comme en témoigne la création du \an{International Journal of Machine Consciousnessen} en 2009, renommé depuis 2020 en \an{Journal of Artificial Intelligence and Consciousness}. 
Plus récemment, Joscha Bach a contribué à fonder le \an{Californian Institute for Machine Consciousness} (CIMC) en argumentant que la recherche sur la conscience des machines était importante :
...
Dans un ouvrage séminal, le philosophe David Chalmers a proposé un découpage célèbre du problème de la conscience en deux niveaux de difficulté: le \an{easy problem} et le \an{hard problem} \cite{chalmers_conscious_1997}.

Le \an{easy problem} est d'expliquer comment un être peut générer des comportements "comme s'il était conscient". 
Il le qualifie de facile non pas parce qu'il pense que nous sommes proches de la solution mais parce qu'il estime qu'il n'y a pas d'obstacle fondamental à sa solution. 
L'étude de ce problème passerait par la construction de robot qui mimeraient la conscience, mais qui ne ressentirait rien. Chalmers parle de zombie philosophique pour décrire de tels êtres. 
Le \an{hard problem} est d'expliquer la subjectivité et les "qualias". 
Il porte sur tout ce que nous n'aurons toujours pas compris même quand nous serons parvenur à construire un robot qui imite la conscience. 


Certains auteurs critiquent ce découpage en estimant que les deux problèmes sont aussi difficiles l'un que l'autre.
Selon eux nous ne parviendrons pas à construire des robots qui miment la conscience si nous ne comprenons pas ce qu'est l'expérience subjective. 
Parmi eux, le philosophe Riccardo Manzotti et le roboticien Antonio Chella on publié une critique de certaines approches de la conscience artificielle qui se situent dans la dichotomie de Chalmers dans leur article  \an{Good Old-Fashioned Artificial Consciousness and the Intermediate Level Fallacy} \cite{manzotti_good_2018}. Ils critique l'utilisation d'un "niveau intermédiaire" ou dans module spécial de l'architecture cognitive qui serait dédié à la conscience. 
Il proposent une conception alternative de la conscience selon laquelle celle ci ne se situe pas dans la tête du sujet mais dans l'interaction entre le sujet et l'objet.  
Ils estiment que la recherche en robotique développementale pourrait aider à progresser dans cette direction.


Mon travail est alignée avec les vues de Manzotti et Chella qui proposent une approche centrée sur les interactions mais ne prétend pas alimenter directement une théorie de la conscience. 
J'aborde néanmoins des sujets connexes comme la perception et la construction de sens. 


\subsection{L'attention}

\noindent
Avec l'article \og \an{Attention is all you need} \fg qui a marqué le début de la technologie des \an{transformers} en 2017 \cite{vaswani_attention_2017}, le concept d'\emph{attention} a fait son entrée remarquée dans la communauté de l'IA. 
Les auteurs y reprennent ce terme à des modèles antérieurs de type \an{attention encoder–decoder}, utilisés en traduction automatique.
Dans ce contexte, il désigne un mécanisme computationnel permettant d'attribuer des poids de saillance différenciés aux éléments d'une séquence en fonction d'un contexte donné. 
...
Dans les sciences cognitives, l'attention est considérée comme une propriété structurelle centrale de l'activité mentale, dans la mesure où le contenu même de la vie mentale est déterminée par ce à quoi le sujet porte attention.
...
Le cerveau des vertébrés est subdivisé en trois grandes régions qui reflètent son organisation développementale au cours de l'embryogenèse : le \emph{cerveau postérieur} (rhombencéphale, \an{hindbrain}), le \emph{cerveau moyen} (mésencéphale, \an{midbrain}), puis le \emph{cerveau antérieur} (prosencéphale, \an{forebrain}), comme illustré en Figure~\ref{fig:cerveau}.
Ce découpage tripartite est associé à trois stades successifs de l’évolution phylogénétique survenus approximativement dans cet ordre. 

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.5\textwidth]{Figures/1_cerveau.png}
	\caption{Division tripartite du cerveau humain qui distingue le cerveau postérieur (\an{hindbrain}), le cerveau moyen (\an{midbrain}), et le cerveau antérieur (\an{forebrain}).}
	\label{fig:cerveau}
\end{figure}

Le cerveau postérieur, le plus ancien dans la phylogenèse et le plus précoce dans l'embryogenèse, se divise en myelencéphale et metencéphale. 
Le myelencéphale assure la régulation des fonctions vitales, telles que la respiration, la pression artérielle, le rythme cardiaque et la digestion. 
Le metencéphale contient le pons et le cervelet. 
Le pons relie le tronc cérébral aux autres structures cérébrales et joue un rôle clé dans le contrôle de certains comportements et réflexes, la régulation de l’état d’éveil et des cycles veille–sommeil.
...
Ce dernier contribue à la prédiction de l’évolution future de la position du corps et des objets en mouvement à partir des informations sensorielles et motrices. 
...
Il contient le tectum (colliculus supérieur et inférieur chez les mammifères), qui joue un rôle majeur dans la localisation spatiale égocentrique de ces signaux sensoriels. 
Le tectum effectue une intégration multimodale fondée sur des coïncidences spatiales et temporelles.
Il est organisé en couches formant des cartes spatiales alignées pour chaque modalité sensorielle, de sorte qu’un même point de l’espace est représenté de manière congruente pour toutes les modalités. 
...
Le cerveau antérieur, dont le développement est le plus récent dans la phylogenèse et le plus tardif dans l'ontogenèse, est formé du diencéphale et du télencephale. 
...
Le télencéphale contient les ganglions de la base, l'amygdale, l’hippocampe et le cortex, qui supportent les fonctions dites de haut niveau telles que la l'interprétation d'information, la prise de décision, la mémorisation, l'apprentissage, la navigation, la mise en \oe uvre d'activités complexes, et, chez l'humain, le langage et le raisonnement abstrait.
...
Lövheim justifie ce modèle en soulignant que les systèmes de contrôle du comportement reposant sur ces trois neurotransmetteurs sont observables à travers l’évolution de nombreuses espèces. 
Ce sont les trois principaux neurotransmetteurs de la famille des monoamines qui en compte plusieurs dizaines. 
Ils sont produits par des petits groupes de neurones dont les corps sont situés dans la partie haute du tronc cérébral et qui projettent leurs axones et diffusent ces neurotransmetteurs à travers tout le cortex. 
L'amygdale, située dans le telencéphale, joue un rôle crucial dans le contrôle de leur production et leur régulation.
Lövheim les décrit comme centraux dans la modulation de comportements sans pour pourtant générer es comportements spécifique à chaque situation mais au contraire en ayant un effet général tout en présentant une dynamique suffisamment rapide pour permettre une adaptation efficace aux changements de l’environnement. 
