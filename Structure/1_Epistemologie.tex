...
Il suggèrent que le cerveau s'appuierait sur des capacités de calcul spatio-temporel héritées de l'évolution phylogénétique, et précoces sur le plan du développement ontogénique, pour naviguer dans des espaces conceptuels abstraits.  
...
Ils rapportent également des études investigant des populations de neurones codant l'espace au delà de la région entorhinal-hippocampal. Certaines de ces études portent sur les \an{goal cells} dans le cortex préfrontal médian.
...
Le mécanisme automatique se baserait sur la forme bordures de l'environnement tels que des surfaces verticales, alors que le mécanisme attentionnel utiliserait des points de repères (landmarks) visuels. 
Certaines études ont meme tenté de déterminer ce qui comptait comme contour de l'environnement: des murs ou mêmes des légères marches sur le sol \cite{cheng_purely_1986}. 
Inversement, des des lignes ou des contours dessinées sur le sol n'étaient pas prise en compte comme contour ni des colonnes espacées.  
 
au moins deux systèmes de navigation: 1) un système pour représenter les distances et les directions de grandes surfaces , et 2) un système des objets et des marques de surfaces pouvant servir de point de balise (\an{landmark}). 
...
Les deux systèmes apparaissent donc pratiquement complémentaire: le système de navigation capture des distances absolue et le sens mais pas les longueurs relatives ni les angles, le système de reconnaissance de forme fait l'inverse. 
...
un système interne qui simule approximativement la dynamique des objets (chutes, collisions, empilements, écoulement de liquides, etc.) pour prédire leur trajectoire .
