...
\noindent
Je ne peux pas clore cette vue d'ensemble de la notion d'intelligence sans aborder les méthodes permettant de l'évaluer et de la quantifier.
...
Le célèbre test de Turing s’inscrit précisément dans cette idée, puisqu’il demande à des observateurs humains de juger la capacité d’une intelligence artificielle à imiter une intelligence naturelle.
Alexei Samsonovich soutient que si la mesure de l'intelligence est formalisée à l'avance alors il est toujours possible de \og hacker \fg le test par un algorithme qui n'adresse pas les questions fondamentales de l'intelligence. 
...
Les tests statiques s'appliquent principalement aux systèmes d'apprentissage supervisés et aux grands modèles d'IA générative. 
...
Il avance l'hypothèse selon laquelle tout agent naturellement intelligent dispose de quatre savoirs fondamentaux hérités de l'évolution phylogénétique (\an{core knowledge}): 
...
Ce benchmark évalue les LLMs en organisant des confrontations en aveugre opposant les modèles deux à deux.
...
Le quadrant \emph{dynamique} / \emph{objectif} s'applique tout particulièrement à l'apprentissage par renforcement puisque celui-ci vise précisément à maximiser une valeur numérique. 
Les jeux de plateau offrent des benchmarks emblématiques dans ce quadran. 
...
Le quandrant \emph{dynamique} / \emph{jugement} regroupe principalement les évaluation de l'inteligence par analyse des comportements en milieu ouvert. 
A la différence des trois autres quadrants, il n'existe pas à ma connaissance de tests qui ont atteint un niveau de célébrité dans le grand public ou de reconnaissance dans la communauté scientifique.   
Il n'existe pas actuellement de compétition inspirée de test de Turing, malgrè des tentatives telles que le Loebner Prize, apparu en 1990 et largement critiqué pour s'arrêter en 2020 \cite{powers_total_1998}.
Dans le domaine des agents en environnement simulé ou des robots, chaque équipe de recherche tend à proposer son propre mode d'évaluation qui se focalise sur les aspects de l'intelligence qui les intéresse. 
...
\begin{itemize}[label=\scalebox{.85}{$\diamond$}]
	\item les \an{non-interactive benchmarks} évaluent la capacité de l'agent à inférer des règles sous-jacentes à partir d'exemples et à les généraliser à de nouveaux cas. Un exemple typique est le ARC challenge présenté plus haut.
