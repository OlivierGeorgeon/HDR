...
\section{Mesurer l'information}

\noindent
Dans la théorie de l'information de Claude Shannon, l'information qu'un agent possède à propos d'une variable $X$ pouvant prendre $n$ valeurs différentes peut être représentée par une distribution de probabilités $d= (p(x_1), p(x_2), ... , \allowbreak p(x_n))$ parfois appelée \an{belief state}. 
L'agent \og croit \fg que $X$ a une probabilité $p(x_i)$ d'avoir pour valeur $x_i$. 
Shannon interprete la grandeur $I(x) = -log_2(p(x))$ comme la quantité d'information (exprimée en nombre de bits) qu'apporte cette valeur une fois qu'elle est connue par l'agent: si l'agent l'attendait, elle apporte peu d'information, s'il ne l'attendait pas, elle en apporte beaucoup.
$I(x)$ peut donc aussi s'interpréter comme la \og surprise attendue \fg  associée à la valeur $x$. 
Plus $x$ est probable (proche de 1), plus l'agent s'attend à une surprise faible (proche de 0) au cas ou elle se produise. 
Si sa probabilité est nulle, la surprise attendue est infinie. 
Sur cette base, Shannon propose de calculer l'entropie de la distribution comme son l'espérence de surprise (\an{expected surprise}), donnée par la somme des surprise attendues pondérées par les probabilités, soit l'Equation \eqref{eq:shannon} :

\begin{equation}\label{eq:shannon}
	H(d) = - \sum_{p(x) \in d} p(x) log_2(p(x))
\end{equation}

Quand la probabilité $p(x_i)$ d'une valeur $x_i$ tend vers 1 et celles des autres tendent vers 0 alors $H(d)$ tend vers 0.
Un \an{belief state} d'entropie nulle représente donc le fait que l'agent croit connaitre avec certitude la valeur de $X$. 
Quand toutes les valeurs sont équiprobables avec la probabilité $p(x_i) = 1/n$, l'entropie est maximale et vaut $H(d) = log(n)$. 
Un \an{belief state} d'entropie maximale représente donc le fait que l'agent croit n'avoir aucune information sur $X$.
%Si le $log$ est en base 2, $H(d)$ indique le nombre de bits nécessaires pour dénombrer les valeurs possibles de $X$. 

L'entropie du \an{belief state} ne donne donc pas d'indication sur la justesse de la croyance de l'agent.


L'entropie est typiquement utilisé à deux niveaux pour évaluer l'apprentissage d'un agent artificiel. 
