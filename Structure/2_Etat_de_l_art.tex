...
L'autonomie constitutive est observée chez les êtres vivants mais la question se pose de savoir si elle est possible chez une entité artificielle telle qu'un robot contrôlé par un ordinateur. 
La notion de \an{plastic structure-determined system} proposé par Maturana \cite{maturana_organization_1975} permet d'imaginer que oui. 
La structure peut etre définie par le sofware du système et dans ce cas l'autonomie constitutive est liée à la capacité à réorganiser ou développer de noueaux éléments de software à fil de l'existence de l'entité. 
En terme informatique, celà  consiste à acquérir du nouveau \emph{code exécutable} plutot que de nouvelles \emph{données}(telles que des paramètres, poids, ou symboles).  
Cette distinction a été identifiée par Terry Winograd et Fernando Flores qui différencie (a) l'ajustement de paramètres, (b) la formation combinatoire de concepts et (c) évolution de structure. 
Il argumentent que c'est seulement quand le système initial n'a pas une structure direcement reliée à la tache envisagée par son concepteur qu'un système computationnel peut produire de nouvelles structures modelées par ses interactions avec l'environnement. 
Ils notent que ceci est valable pour \og any system whose internal structure can change as a result of perturbations, and computer programs share this quality \fg.
De tels software flexible pourrait dépasser les limitations de leurs programmes initiaux. 
Alexander Riegler a aussi argumenté que le couplage structurel n'est pas nécessairement limité au domaine physique, mais il les software informatiques peuvent aussi devenir "incarnés" et construire leurs propres objectifs qui sont nouveaux par rapport au système initial défini par le programmeur. 

Le concept d'auto-programmation (\an{self-programming}) est n'est pas nouveau. 
Allan Newel et Herbert Simon l'ont anticipé dans leur lecture fondatrice donnée à leur \an{Turing Award} \cite{newell_computer_1976}. 
...
La question de comment cela peut être mise en \oe uvre est encore complètement ouverte. 
En particulier elle soulève de nombreuses questions parmi lesquelles: 
