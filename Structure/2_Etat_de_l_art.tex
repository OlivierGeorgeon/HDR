...
La sémantique est l'étude de comment la vérité et le sens sont affectés à des mots et des énoncés. 
Wang note qu'en mathématique et en intelligence artificielle, la plupart des sémantiques sont basées sur un modèle \an{model-theoretic}, en référence à la \an{foundation of semantics} d'Alfred Tarski \cite{tarski_semantic_1944}.
Une sémantique basée sur un modèle d'un langage $L$ implique un modèle $M$ et une interprétation $I$ qui met en corresondance les mots de $L$ avec les objets et les relations de $M$: ``the meaning of a word in L is defined as its image in M under I, and whether a statement in L is true is determined by whether it is mapped by I onto a fact in M'' \cite[p.~284]{wang_experience-grounded_2005}. 

En intelligence artificiellen, cependant, une sémantique basée sur un modèle nécessite que le concepteur fournisse le modèle à l'agent. 
Dans ce cas, Wang dit que la sémantique ``tells us what the words mean to us, but says nothing about what they mean to the system'' \cite[p.~284]{wang_experience-grounded_2005}

Contrairement à une sémantique basée sur un modèle, une sémantique basée sur l'expérience définit la vérité et le sens des expressions en fonction de leur role dans l'historique des interactions entre l'agent et l'environnement. 
...
Wang a proposé une implémentation d'une sémantique ancrée sur l'expérience dans son Non Axiomatic Reasoning System (NARS). 
