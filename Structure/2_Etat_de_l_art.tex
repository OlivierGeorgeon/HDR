...
Cet encrage renvoie davantage à un fondement sous-jacent plutôt qu'à un point d'attache évitant une dérive. 
Aux premiers temps de l'IA, les systèmes à base de règle ont été critiqués pour manipuler des connaissances \og hors sol \fg , mais aujourd'hui il est possible d'argumenter que les systèmes d'apprentissage machine sont ancrés dans leurs données d'entrainement: par exemple, les LLMs seraient ancrés dans des corpus de texte. 
...
NARS les mets à jour chaque fois qu'il reçoit un énoncé qui confirme ou infirme cette relation par des \an{positive} ou des \an{negative evidences}. 
...
Ron Sun \cite[\an{Symbol grounding: A new look at an old idea}]{sun_symbol_2000} a argumenté que l'ancrage du sens dans l'expérience impliquait l'acquisition d'habitudes sensorimotrices.
Il se réfère à la phénoménologie de Heidegger évoquée en Section \ref{sec:phenomenologie} pour argumenter que le sens s'ancre dans les habitudes d'interactions acquises.  
Selon la phénoménologie, ses habitudes sont prises sur la base de tendances à interagir avec les choses qui ne nécessitent pas d'objectifs prédéfinis ni de représentations explicites.   
Dileep George et ses collègues ont proposé une implémentation de cette idée \cite[\an{Behavior is everything: Towards representing concepts with sensorimotor contingencies}]{hay_behavior_2018} dans laquelle les concepts sont représentés comme des séquences d'événements d'interaction sensorimotrices.
...
La question de comment un agent peut construire du sens à partir de son expérience sensorimotrices est une question fondamentale de l'intelligence qui reste ouverte. 
Cette question peut s'inscrire dans le champ de la sémantique si on conçoit cette discipline dans son acception large, c'est à dire, comme le suggère Wang, un cadre de pensée dans lequel les concepts de signification et de vérité ont un sens. 
Cette sémantique basée sur l'expérience suggère que la signification serait ancrée dans l'usage sensorimoteur, dans la lignée d'une épistémologie pragmatique introduite en section \ref{sec:pragmatisme}. 
