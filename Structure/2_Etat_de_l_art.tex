...
\noindent
La représentation bayésienne offre un modèle complémentaire au cycle de décision discret qui prend en compte la diversité des entrées et sorties et leur possible parallélisme.
...
Les réseaux bayésiens, parfois appelés \an{belief networks}, sont des graphes acycliques directs dont les n\oe uds représentent des variables aléatoires et les arcs des dépendances probabilistes entre ces variables. 
...
Pour cela le robot doit connaitre la distribution de probabilités $P(O|S)$ que le capteur soit dans un certain été (i.e., impact) quand l'environnement est dans un certain état (i.e., objet), les distributions de probabilités dites \emph{marginales}: $P(S)$ de chaque état possible de l'environnement, et $P(O)$ de chaque observation.

Cette forme d'inférence baysienne suppose de disposer, en amont, d'une modélisation des observations et des états possibles de l'environnement. 
...
Sans la mobiliser directement, je reconnais deux apport intéressants à la représentation bayésienne. 
D'une part, elle permet de prendre en compte l'influence parallèle des différentes entrées et sorties au sein d'un même réseau, ce qui offre un alternative au modèle d'alternance entre entrées et sorties. 
D'autre part, elle pose le problème fondamental d'inférer une connaissance du monde comme la meilleure hypothèse explicative (\an{best guess}) sur le monde que l'agent puisse formuler pour rendre compte la dynamique de ses interactions. 
Elle propose de réaliser cette inférence à partir d'observations qui ne constituent pas en elles-même une représentation directe de l'état du monde, que je qualifierai d'\emph{observations non représentationnelles}.

\subsection{Couverture de Markov}


\noindent
Pour modéliser l'évolution des variables aléatoires au cours du temps, on peut utiliser un Réseau Bayésient Dynamique (\an{ Dynamic Bayesian Network}, DBN) \cite{murphy_dynamic_2002}. 
Les DBN sont des extensions de BN définis par un réseau bayésien \an{prior}  $B_0$,  et un réseau bayésien \an{transition} $B_T$ comme illustré dans la partie gauche de la Figure \ref{fig:dbn}.
...
La partie droite de la Figure \ref{fig:dbn} montre une représentation \emph{circulaire réduite} du DBN qui ne met pas en évidence la structure exacte de la dépendance temporelle. 
...
La dynamique d'une \emph{entité} (vivante ou non vivante) dans un environnement peut être représentée par un DBN, dont un sous-ensemble $V_0$ des variables représente l'entité, et son complément $V_m$ représente l'environnement. 
Un $B_T$ avec une profondeur temporelle d'un seul pas de temps est généralement suffisant en supposant que l'état de l'entité et de l'environnement au temps $t$ découle de leur état au temps $t-1$ uniquement.
Certaines variables peuvent représenter des vitesses ou des mémoires. 
...
Dans le cas ou l'entité est un organisme vivant, sa couverture de Markov se scinde en deux sous ensembles de variables selon que leurs relations de dépendance vont de l'intérieur vers l'extérieur ou l'inverse. 
La Figure \ref{fig:blanket}, provenant de l'article de Alexander Ororbia et Karl Friston intitulé \og \an{Mortal Computation: A Foundation for Biomimetic Intelligence} \fg \cite{ororbia_mortal_2024} donne une représentation circulaire réduite d'un tel modèle.
