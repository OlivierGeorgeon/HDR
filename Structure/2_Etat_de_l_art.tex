...
Il existe de nombreuses spécialisations des réseaux bayésiens adaptés à différent types de problèmes, par exemple les chaines de Markov cachées et les filtres de Kalman.
%et les réseaux bayésiens. 


La Figure \ref{fig:bn} montre un réseau bayésien minimal contenant deux variables $S$ et $O$ liées par une relation d'influence signifiant que la probabilité que la variable $O$ prenne une valeur particulière $o$ dépend de la valeur $s$ prise par la variable $S$. 
...
Le processus d'inférence bayésienne consiste à calculer les distributions de probabilité des autres variables qui leurs sont liées, jusqu'à converger vers une distribution de probabilité conjointe (\an{Joint Probability Distribution}, JPD) de l'ensemble des variables du réseau.  

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/2_dbn.pdf}
	\caption{Réseau bayésien dynamique.}
	\label{fig:dbn}
\end{figure}

Une \emph{entité} (vivante ou non vivante) est représentée par un sous-ensemble $V_0 \subset V$ de variables connexes identifiées au sein du réseau. 
Les variables appartenant à $V_0$ qui ont des parents ou des enfants dans le reste du réseau définissent l'interface entre l'entité et son environnement, aussi appelée la   \emph{couverture de Markov} de l'entité notée $V_m \subset V_0$. 
...
L'entité est comprise comme un \emph{observeur actif} de son environnement car il agit et perçoit simultanément. 
La modélisation bayésienne voit la dynamique et l'auto-organisation de l'observeur comme maintenant des croyances probabilistes représentées par les distributions de probabilité des variables de $V_0$ ($Z$, $S$, et $A$ en Figure \ref{fig:blanket}) à propos de l'\emph{observé} représenté par les distributions de probabilité des variables de l'environnement  ($O$ en Figure \ref{fig:blanket}).
...
L'état des variables internes de l'organismes $V_0$ représente finalement la meilleure hypothèse (\an{best guess}) que l'organisme peut faire à propos de l'état des variables cachées de l'environnement pour expliquer la dynamique de ses expériences d'interaction.
...
Le concept de couverture de Markov est très employé en biologie pour signifier qu'il est mathématiquement impossible pour un organisme d'inférer la structure exacte du monde extérieur.
...
Par exemple, Gary Drescher l'exprime ainsi: 
\begin{quote}
...
Legg \cite{legg_universal_2007} illustre le problème de l'inférence par un exemple simple. 
...
Il porpose que les systèmes d'intelligence artificielle devraient suivre le même principe.
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/active_inference.jpg}
	\caption{Théorie de l'inférence active \cite[Fig.~1]{pezzulo_active_2024}.
	Le monde a un état $x*$ caché à l'agent qui génère une observation $y$.
	Notre modèle interne $x$ explique l'observation $y$ 
	}
	\label{fig:actu}
\end{figure}
