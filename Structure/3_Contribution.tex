...
\noindent
Je propose le modèle \an{Enactive Decision Process} (EDP) pour formaliser les interactions d'un agent dont les signaux sensoriels sont non-représentationnels et qui ne reçoit pas de récompense externe.
Le EDP est représenté en Figure~\ref{fig:edp} et formalisé dans la Définition \ref{def:edp}. 
...
La première est que le cycle commence par la décision de l'agent, ce qui est matérialisé par la puce noire sur la figure.
La seconde est que le signal sensoriel est un événement d'interaction, nommé $i_t$,  qui intègre une information sur la décisions qui l'a précédé.
...
\begin{figure}[ht]
	...
	\caption{Enactive Decision Process.
		Au temps $t$, l'agent prend une décision $d_t$, puis reçoit un signal sensoriel $i_t$ selon une distribution de probabilité $q$ conditionnelle à l'état de l'environnement $s_t$ et à la décision $d_{t}$.
		L'environnement tansitionne vers l'état $s_{t+1}$ selon une distribution de probabilité $p$ conditionnelle à $s_t$ et $i_t$.
	}
	\label{fig:edp}
\end{figure}
...
Le signal sensoriel  $i_t$ est tirée avec la probabilité $q(i_t|s_t,d_t)$. 
...
Par rapport au POMDP, le modèle EDP présente l'avantage de rendre clairement visible le fait que le signal sensoriel peut dépendre de la décision de l'agent.
Cela rappelle au chercheur de ne pas implémenter l'hypothèse de représentationnalité.
Cette dépendance n'interdit cependant pas que certains signaux sensoriels puissent être représentationnels. 
Le fait que la décision et le signal sensoriel qui en résulte soient rattachés au même cycle $t$ évite certaines lourdeurs du POMDP évoquées en Section~\ref{sec:pomdp}.
L'historique $h_t$ des interactions au temps $t$ est également simplifié sous la forme d'une série constitué d'une seule variable $h_t = \langle i_0, ..., i_{t} \rangle$, au lieu des deux variables $d_t$ et $o_t$ dans le cas des POMDPs.
...
Le terme Markov a ensuite été retiré pour indiquer que, bien qu'il s'inspire du POMDP, ce modèle n'a plus rien de markovien.
...
La formalisation en DBN a été réalisée avec les étudiants en mathématique Marouane Toumi, Pierre Martin et David Lurie \cite{georgeon_artificial_2024}.
