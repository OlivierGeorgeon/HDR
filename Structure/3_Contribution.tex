...
Nous allons maintenant examiner comment implémenter ce mécanisme chez un agent artificiel.  
...
Dans le cas des expérimentations en environnement simulé, les interactions primitives correspondent à des couples \schema{\emph{décision}, \an{input}}, dans lequel l'\an{input} est un signal sensoriel non représentationnel transmis de l'environnement à l'agent.
...
%Il fonctionne également directement pour contrôler des robots dans le monde physique comme je l'explique à la fin de cette section. 
...
Les autres décisions peuvent donner lieu à deux interactions différentes selon que l'agent interagit avec une cellule vide ou un mur, différentiés par un \an{input} de 1 bit.
Cet \an{input} constitue un signal sensoriel non représentationnel dans la mesure ou ce n'est pas nécessairement la même valeur du bit qui indique un mur ou une cellule vide d'une interaction à l'autre.
...
A partir de cette inspiration, nous avons modélisé les interactions sur la base de trois décisions et sept \an{inputs} possibles. 
...
Les \an{inputs} \texttt{stable}, \texttt{decrease} et \texttt{increase} indiquent une variation d'odeur entre la cellule reniflée et la cellule où se situe l'agent dans le cas des décisions \texttt{sniff}, ou liée au déplacement de l'agent dans le cas où il avance ou tourne.
...
L'agent possède les mêmes interactions primitives que précédement mais elles sont complétées par leurs positions $\epsilon$ et leurs isométries $\iota$ indiquées dans le Tableau \ref{tab:small_loop_spatial}.
...
\noindent
Cette expérimentation examine comment générer des comportements plus sophistiqués dans un environnement dynamique et continu contenant différents types d'objets.
Nous avons étendu les mécanismes perceptifs et motivationnels d'ECA, et ajouté un \emph{mécanisme attentionnel}.

La Figure~\ref{fig:requins} présente les conditions expérimentales.
...
La position et la direction de l'agent sont régis par une loi de frottement fluide qui fait qu'il ralentit progressivement après chaque battement de nageoire sans pour autant s'arrêter avant le coup de nageoire suivant.
Les impulsions vers l'avant le déplacent d'environ une unité et chaque impulsion de rotation le fait tourner d'environ $\pi / 4$.
...
L'attention de l'agent est attirée par les objets saillants présents dans son champ visuel: poisson, autre agent, mur coloré ou algue.
Au début, son attention se porte sur l'objet le plus proche et l'agent apprend un comportement d'approche comme dans l'expérimentation de la limule. 
...
Une fois les \an{bundles} créés, l'agent porte préférentiellement son attention sur les objets qui affordent des interactions qui ont une valence positive. 
Ainsi, l'agent apprend à chasser les poissons quand il les voit et à interagir avec l'autre agent comme s'il voulait jouer avec lui.
...
\begin{figure}[ht]
	...
	\caption{Environnement continu dynamique. 
		...
		Poissons bleus: cibles.
		Cellules vert-foncées: murs.
		Cellules colorées: éléments de murs identifiable.
		Trèfles: algues identifiables pouvant être traversées.}
	\label{fig:requins}
\end{figure}

Cette expérimentation a été rapportée avec plus de détails dans un article de Simon Gay avec Jong Kim \cite{gay_implementing_2012}. 
Nous avons utilisé une approche similaire dans une autre expérimentation mettant en évidence un comportement de navigation de \an{landmark} en \an{landmark}. 
L'agent disposait d'un mécanisme motivationnel plus sophistiqué qui lui faisait porter son attention sur des objets servant de point de repère successifs pour retrouver son chemin \cite[\an{Early-Stage Vision of Composite Scenes for
	Spatial Learning and Navigation}]{georgeon_early-stage_2011}. 
Avec Mathieu Guillermin, nous l'avons utilisée comme exemple dans un article récent \cite{guillermin_artificial_2022}.
...
L'usage de ces signaux est étroitement lié au mécanisme attentionnel qui est impliqué dans la reconnaissance d'objets et la sélection des comportements.
Le mécanisme d'attention, associé aux valences motivationnelles joue un rôle crucial dans l'apprentissage de l'existence des objets et l'organisation des comportements autour de ces objets, comme les théories de présentées en Section~\ref{sec:attention} le suggèrent.
