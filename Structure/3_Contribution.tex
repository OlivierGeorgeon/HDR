...
Pour rendre compte de la façon dont les organismes effectuent une inférence enactive dans le monde physique, nous proposons le formalisme du \textit{Processus de Décision Enactif Spatial} (PDES).
...
Le PDES est une spécialisation du PDE qui encode l'hypothèse que l'ensemble des états $S$ est un \textit{monde tridimensionnel}.
Un monde 3D est un espace euclidien tridimensionnel dans lequel les objets sont représentés comme des ensembles de points sur lesquels des transformations affines géométriques peuvent être effectuées.
Un espace euclidien 3D est un espace vectoriel réel tridimensionnel $\mathcal{E} = \mathbb{R}^3$ associé à la distance euclidienne.
Nous définissons un PDES en étendant le modèle PDEM sous la forme d'un tuple $\Gamma_S=(S, D, I, \mathcal{E}, \mathcal{I}, p, q)$ où $\mathcal{E}$ est un espace euclidien 3D, et $\mathcal{I}$ est l'ensemble des isométries directes sur $\mathcal{E}$.
Une isométrie directe, ou \textit{mouvement rigide}, est une transformation affine qui préserve les distances et les angles, c'est-à-dire une translation, une rotation, ou une combinaison des deux.
Comme pour le cycle PDEM introduit dans la Section \ref{sec:emdp}, le cycle PDES est illustré dans la Figure \ref{fig:semdp} et se déroule comme suit :
\begin{itemize}
	\item Le PDES est initialisé avec un état initial $s_0 \in S$.
	\item À chaque pas $t \geq 0$,
	l'agent prend une décision $d_t\in D$.
	L'interaction enactée $i_t \in I$, sa position $\epsilon_t \in \mathcal{E}$, et l'isométrie $\iota_t \in \mathcal{I}$ sont tirées avec la probabilité $q(i_t, \epsilon_t, \iota_t|s_t,d_t)$.
	L'état suivant $s_{t+1}$ est tiré avec la probabilité $p(s_{t+1}|s_{t},i_{t}, \epsilon_t, \iota_t)$.
\end{itemize}
L'agent implémente l'hypothèse qu'il contrôle un robot mobile dans le monde 3D.
...
Cette hypothèse de monde 3D trouve son appui dans l'observation que la plupart des animaux peuvent localiser leurs interactions dans leur environnement immédiat et détecter leur propre déplacement (par exemple, via le signal vestibulaire et le flux optique).
On notera que l'agent n'a pas accès à une quelconque « position absolue » supposée du robot dans l'environnement.
