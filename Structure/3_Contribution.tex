...
\noindent
Au cours de la revue de l'état de l'art du Chapitre \ref{chap:etat}, j'ai évoqué que les signaux sensoriels pouvaient avoir des statuts différents selon les modèles.  
Cette section vise à clarifier ces différences en introduisant la distinction entre signal sensoriel \emph{représentationnel} et signal sensoriel \an{non-représentationnel}. 

Intuitivement, je qualifie un signal sensoriel de \emph{représentationnel} s'il représente une propriété possédée par un objet de l'environnement. 
Des exemples sont les signaux sensoriels qui représentent la couleur, la dureté, le goût, ou la forme d'un objet.
Si ce n'est pas le cas, alors le signal sensoriel est non-représentationnel; 
par exemple, un signal qui indique un choc avec un objet, un changement de luminosité, ou l'apparition d'une odeur. 
D'une manière générale, les signaux représentationnels reflètent des propriétés ontologiques associées aux objets présents dans l'environnement, alors que les signaux non-représentationnels signalent des événements.  

Plus formellement, un signal sensoriel $o$ est représentationnel, si ce sont toujours les mêmes états de l'environnement qui engendrent ce signal.
Nous pouvons formaliser cette propriété dans le cas d'un environnement fermé dont l'ensemble $S$ des états est connu, et d'une fonction d'observation déterministe. 
Définissons:
...
%$\exists \{S_1, S2\} \in \mathcal{P}(S) ,\quad \forall t \in T,\quad \forall s \in S_1,\quad \mathcal{O}(s) = o_t$ \text{et} $\forall s \in S_2,\quad \mathcal{O}(s) \ne o_t $

Le signal sensoriel $o \in O$ est représentationnel si et seulement si il existe une partition $\{S_1, S_2\}$ de $S$ telle qu'a chaque instant, tous les états de $S_1$ engendrent le signal $o$, et aucun état de $S_2$ ne l'engendre:
...
Si la condition est satisfaite, $o$ représente une propriété de l'environnement qui est possédée par tous les états appartenant à $S_1$ et qui est absente des états appartenant à $S_2$.

De manière similaire, je qualifie une fonction d'observation stochastique de représentationnelle si elle donne toujours la même probabilité de signal sensoriel pour chaque état possible de l'environnement. 
Cela signifie que le signal sensoriel représente une propriété possédée par certains états de l'environnement mélangé à un bruit aléatoire introduit par la stochasticité. 
Une fonction d'observation stochastique représentationnelle est donc de la forme: 
$\mathcal{O} : S \to \Delta(O) $  donnant la distribution de probabilité  $ \mathcal{O}(o_t| s_t)$ d'obtenir le signal $o_t$ dans dans l'état $s_t$.

Un exemple de fonction d'observation stochastique non représentationnelle est la fonction utilisée dans la modélisation générale des POMDPs introduite en Section \ref{sec:pomdp}: $q:S\times S\rightarrow\Delta(O)$ qui donne la distribution de probabilité $q(o_{t}|d_{t-1}, s_t)$ des observations $o_{t} \in O$ conditionnelle à l'état $s_t\in S$ et à la décision précédente $d_{t-1}$.
...
Le statut représentationnel peut être rapproché du statut markovien introduit en Section \ref{sec:rl}. 
...
Ils sont non-représentationnels quand ils constituent un \an{feedback} résultant de l'action.


\subsection{Enactive Decision Process (EDP)}
\label{sec:edp}
