...
Dans le cas des expérimentations en environnement simulé, les interactions primitives correspondent à des couples \schema{décision, \an{input}}, dans lequel l'\an{input} est un signal sensoriel non représentationnel transmis de l'environnement à l'agent.
...
L'agent dispose de six décisions possibles : \texttt{move forward}: tenter d'avancer d'une cellule,  \texttt{turn left} : tourner 90° à gauche, \texttt{turn right} : tourner 90° à droite, \texttt{feel left} : toucher la cellule à gauche, \texttt{feel front} : toucher la cellule devant, \texttt{feel right} : toucher la cellule à droite.
...
Cet \an{input} constitue un signal sensoriel non représentationnel dans la mesure ou ce n'est pas nécessairement la même valeur du bit qui indique un mure ou une cellule vide d'une interaction à l'autre.
...
\texttt{increase right} : la cible apparait ou grossit respectivement à droite à gauche, en face, ou à droite,
...
Les deux champs visuels se recouvrent et permettent de détecter la présence de la cible devant l'agent.
...
Les décisions de rotation provoquaient un mouvement de rotation pendant une durée correspondante à une rotation d'environ 90 degré. 
...
Martin Kody\v{s} et Christian Wolf ont implémenté les interactions primitives de l'expérience de la limule chez robot Eddie de la Figure~\ref{fig:robot_serie} (droite) \cite{kodis_approche_2014}.   
...
\noindent
Cette section examine la possibilité de permettre à un agent de développer des comportements plus intelligents lorsqu'on le dote de mécanismes innés lui permettant de traiter certains effets spatiaux liés à ses interactions. 
...
\begin{De}[SEDP]
	\label{def:sedp}
	Un \an{Spatial Enactive Decision Process} (SEDP) se définit par un tuple EDP $(S, D, I, p, q)$, complété par l'espace tridimensionnel $\mathbb{R}^3$ et l'ensemble $\mathcal{I}$ des isométries directes définies sur $\mathbb{R}^3$, dans lequel:
	\begin{itemize}[label=\scalebox{.85}{$\diamond$}]
		\item $S$ est un ensemble d'états,
		\item $D$ est un ensemble fini de décisions,
		\item $I$ est un ensemble fini d'événements d'interaction,
...
\noindent
Nous avons doté l'agent d'une architecture cognitive qui implémente l'hypothèse que l'environnement possède une structure de \emph{monde physique euclidien}.
Un monde physique eucliden est un espace euclidien contenant des objets, comme par exemple notre monde physique naturel ou des environnement virtuels simulés.
Cette architecture dispose d'une mémoire spatiale égocentrée dans laquelle elle peut positionner les événements d'interaction. 
Elle peut mettre à jour cette mémoire spatiale en lui appliquant des isométries directes.  

Les interactions entre l'agent et son environnement sont modélisées par le SEDP. 
A chaque cycle d'interaction, l'architecture met à jour l'état de sa mémoire spatiale en fonction du \emph{déplacement supposé} de l'agent et ajoute la nouvelle interaction énactée à sa \emph{position supposée}. 
Le déplacement supposé est informé par l'isométrie $\iota_t$, et la position supposée par le point $\epsilon_t$.


Par exemple, il contrôle un robot mobile dans le monde 3D.
Le point $\epsilon$ et l'isométrie $\iota$ indiquent respectivement la position de l'interaction enactée $i$ et le déplacement du robot durant l'enaction de l'interaction.
$\epsilon$ et $\iota$ sont tous deux encodés en coordonnées égocentriques relatives au corps du robot.
