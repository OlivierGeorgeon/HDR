...
\noindent
Je propose le modèle \an{Enactive Decision Process} (EDP) pour formaliser les interactions d'un agent dont les signaux sensoriels non représentationnels et qui ne reçoit pas de récompense externe.
Ce modèle intègre le fait que le signal sensoriel n'est pas une représentation partielle du monde mais dépend également de la décision qui l'a précédé comme nous l'avons vu à la section précédente. 
Il s'appuie sur le modèle du POMDP de la définition \ref{def:pomdp}, en mettant en évidence que la décision impacte le signal sensoriel et en éliminant la fonction de récompense qui provenant de l'environnement.
...
%Par exemple, la décision $d_1$ peut consister à essayer d'avancer pendant une seconde. En fonction de l'environnement, il peut en découler une interaction $i_1$ qui consister à avoir effectivement avancé pendant une seconde ou alors une interaction $i_2$ qui consiste à percuter un obstacle.En comparaison avec le modèle POMDP, le cycle d'interaction a été inversé: l'agent ne reçoit plus une observation $o_t$ en début de cycle mais une \emph{interaction enactée} $i_t$ en fin de cycle, résultante de la décision $d_t$. Notons que dans les deux cas, $o_t$ et $i_t$ sont non markoviennes car elles ne constitue pas un \an{state signal} suffisant pour prendre la prochaine décision. L'historique des interactions enactées est la seule source d'information de l'agent à propos du monde. 

%Une autre différence est que l'agent ne reçoit plus de récompense $r_t$ provenant de l'état. Un agent EDP n'est pas destiné à atteindre des états auxquels sont associées des récompenses positives. Il n'observe pas des propriétés partielles et bruitées d'un processus markovien dont il pourrait reconstituer l'état caché connaissant a priori les probabilités de transition. L'EDP a vocation à être utilisé quand on ignore les distributions de probabilité $p$ et $q$. On ne peut donc pas construire la fonction de mise à jour du \an{beleif state} comme présenté en Section \ref{sec:pomdp} sur les POMDPs. 
