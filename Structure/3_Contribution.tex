...
comme nous allons le voir dans des exemples. 
...
L'environnement calcule les effets de cette décision pour se mettre à jour, puis il génère l'\an{outcome} qu'il transmets à l'agent via l'\an{input} du DDS. 
...
Dans ce ca, la seule différence du \an{software} de l'agent se situe au niveau du contrôleur primitif qui implémente la mise en \oe uvre des décisions de l'agent à travers des boucles de contrôle sensorimotrices.
...
L'agent dispose de six décisions possibles : \texttt{move forward}: tenter d'avancer d'une cellule,  \texttt{turn left} et \texttt{turn right} : tourner 90° à gauche et à droite, \texttt{feel left}, \texttt{feel front}, \texttt{feel right} : toucher la cellule à gauche, devant, et à droite.
...
Ces schémas de haut niveau sont pris en défaut quand il arrive dans le coin supérieur droit de la boucle. 
...
La Figure \ref{fig:horseshoe} illustre le système de perception distal de l'agent.  
...
Mais ces comportements nécessitent un certain apprentissage car l'agent ignore initialement les relations entre ses décisions et les interactions produites. 
...
L'agent ne fait plus de collision après le cycle 260, et mange la première cible au pas 380, puis de nouvelles cibles aux pas 605 et 685.
La courbe bleue montre qu'il réénacte une interaction composite de 10 pas au cycle 450. 
La courbe orange montre que la croissance du nombre de schémas reste contenue et ralentit avec le temps. 
...
La Figure \ref{fig:attention}  montre une analyse des structures apprises par le \an{transformer}.
La partie gauche montre une projection en trois dimensions de l'espace d'\an{embedding}. 
Nous avons examiné s'il existait des regroupements de \an{tokens} qui pourraient refléter des proximités sémantique mais ce n'est pas le cas. 
L'analyse des matrices d'attention s'est révélée plus concluante. 
La partie droite de la Figure \ref{fig:attention} montre un exemple obtenu avec la séquence 
...
La décision d'avancer déclenchait l'avancement du robot pendant une durée correspondant à un déplacement d'environ 5 cm tout en maintenant la LED infrarouge frontale allumée.
...
Un QR code était utilisé comme cibles. 
...
Elles mettent en lumière le besoin de disposer de plateformes modulaires autorisant l'ajouter de capteurs adaptés à l'apprentissage sensorimoteur.
...
\noindent
Cette section examine comment permettre à un agent de développer des comportements plus intelligents lorsqu'on le dote de mécanismes innés lui permettant de traiter des régularités spatiales interactions. 
Je propose le \emph{Processus de Décision Enactif Spatial} pour modéliser un agent recevant des informations spatiales sur ses interactions énactées. 
...
Ces informations spatiales sont centrées sur le corps de l'agent; il  n'a pas accès à une quelconque \emph{position absolue} dans son environnement.
...
Les variables représentant l'interaction $I$, le point $\mathcal{E}$, et l'isométrie $\mathcal{I}$ constituent la couverture de Markov de l'agent.
ECA vise à encoder des présupposés qui permette à l'agent de construire un modèle qui rende compte de la structure physique euclidienne du monde. 
Comme évoqué en Section~\ref{sec:couverture}, nous estimons que ces présupposés sont nécessaire pour inférer les structures de l'environnement cachées par la couverture de Markov.
...
Au centre vient s'ajouter la \emph{mémoire de travail spatiale égocentrée}, et à gauche une \emph{mémoire proto-ontologique} à long terme qui sont toutes deux initialement vides. 
...
La mémoire spatiale est définie comme un ensemble de d'\emph{interactions localisées}.
...
Les interaction énactées réactivent les \an{bundles} précédemment appris auxquels elles appartiennent ce qui provoque un effet \emph{d'évocation} du phénomène. 
...
\noindent
Cette expérimentation place un agent contrôlé par ECA dans l'environnement du \an{Small Loop Challenge} introduit en Section~\ref{sec:small-loop}.
Elle vise à mettre en évidence le processus de construction des \an{bundles}.
...
Les positons $\epsilon$ définissent le \emph{modèle corporel} de l'agent (\an{body model}), et les isométries $\iota$ son \emph{modèle de déplacement} (\an{displacement model}).
...
La figure~\ref{fig:comportement_ECA} présente une analyse des comportements obtenus dans une exemple d'exécution.
...
Le \an{bundle} représentant la cellule vide est créé au pas 5 quand l'agent observe que l'interaction \interaction{move forward, empty} énactée au pas 5 se chevauche avec \interaction{feel front, empty} énactée au pas 1.
...
Le \an{bundle} représentant un mur est créé au pas 13 lorsque l'agent observe que l'interaction \interaction{move forward, wall} énactée au pas 13 se chevauche avec \interaction{feel front wall} énactée au pas 24.
Il est complété jusqu'au pas 32 où il content les quatre interactions primitives affordées par un mur. 


\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/3_Trace.pdf}
	\caption{Analyse des comportements \cite[Fig.~8]{georgeon_eca_2013}.
...
L'agent ECA parvient à éviter les collisions et à obtenir des comportements satisfaisants plus rapidement qu'avec le mécanisme de schéma uniquement. 2
...
Elle soulève de nombreuses questions sur la gestion des \an{bundles}.  
...
Nous avons étendu les mécanismes perceptifs et motivationnels d'ECA en y ajoutant une \emph{mécanisme attentionnel}.

La Figure~\ref{fig:requins} illustre les conditions expérimentales.
%L'environnement comprend deux agents ECA (les requins gris), des murs (cellules pleines pouvant arborer des parties colorées identifiables), des cibles mobiles (poissons bleus se 
déplaçant en ligne droite et changeant de direction au contact d'un mur), ainsi que des algues (les \og trèfles \fg), repères visuels que les agents peuvent traverser.
...
Le poissons bleus sont des cibles mobiles qui se déplacent en ligne droite et changent de direction quand ils rencontrent un mur.
Les \og trèfles \fg\ sont des algues identifiables visuellement que les requins peuvent traverser.  
...
L'attention de l'agent est attirée par les objets saillants de son champ visuel: poisson, autre agent, mur coloré ou algue.
...
Les deux agents tendent aussi à se rapprocher l'un de l'autre ce qui génère une forme de comportement ludique.
...
Cette expérimentation a été décrite en détail dans un article de Simon Gay avec Jong Kim \cite{gay_implementing_2012}. 
...
Le couplage entre le mécanisme d'attention et le système motivationnel basé sur les valences permet l'apprentissage de l'existence des objets et l'organisation des comportements autour de ces objets, comme les théories de présentées en Section~\ref{sec:attention} le suggèrent.
