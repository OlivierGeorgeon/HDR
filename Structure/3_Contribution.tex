...
Lorsque l'agent interagit avec un environnement simulé, ce principe est simplifié par le fait que l'environnement peut lui-même interpréter la décision de l'agent.  
Dans ce cas, un cycle du couplage primitif correspond à un cycle \an{output} -- \an{input} du DDS, comme nous allons le voir dans des exemples. 
...
\noindent
Cette section présentent trois expérimentations significatives réalisées avec le mécanisme de schéma. 
La seule différence au niveau du \an{software} de l'agent se situe au niveau de leurs contrôleurs primitifs qui implémentent des boucles sensorimotrices différentes. 
Du fait que le mécanisme de schéma est agnostique sur la signification des décisions et des interactions primitives, il s'adapte aux différentes situations. 

Dans le cas des expérimentations en environnement simulé, chaque interaction primitive correspond à un couples \schema{\an{decision}, \an{outcome}}. 
La décision de l'agent est transmise à l'environnement via l'\an{output} du DDS.
L'environnement simule les effets de cette décision puis il génère un résultat appelé \an{outcome} qu'il transmets à l'agent via l'\an{input} du DDS. 
Cet \an{outcome} est une rétroaction non représentatinnelle résultant de la décision. 
Il ne reste plus au contrôleur primitif qu'a associer la décision avec le résultat pour produire l'interaction primitive énactée $i=$ \schema{\an{decision}, \an{outcome}}.

%Dans le cas des robots, les interactions primitives sont générées à partir de boucles \an{output}--\an{input} qui seront détaillées en Section \ref{sec:sequentiel-robotique}.
