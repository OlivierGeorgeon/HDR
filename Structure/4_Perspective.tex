...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|c|}
		...
		\texttt{watch} & rester immobile & durée (ms) \\ \hline
...
La décision \texttt{watch} consiste à rester immobile et effectuer périodiquement des mesures télémétriques pendant la durée définie dans la variable \texttt{duration}. 
...
L'événement \texttt{floor contrast} contient un code indiquant de quel coté une ligne noire a été détectée pendant le déplacement. 
Ce code peut prendre les valeurs \texttt{none}, \texttt{left}, \texttt{front} et \texttt{right}.
Cet événement est placée à la position central de la barre de capteurs de luminosité à laquelle s'ajoute la distance de recule réflex provoqué par la détection d'un contraste de luminosité sur le sol. 
...
Cette mémoire allocentrée est capable de créer et maintenir un graphe dont les n\oe uds représentent les lieux visités et les arcs des passages d'un lieu à l'autre. 

Nous avons implémenté des comportements innés basés sur deux états émotionnels distincts: L'état \texttt{interest} pousse l'agent à énacter des \emph{comportements d'attention} lui permettant de reconnaitre un lieu, et l'état \texttt{enjoyment} pousse l'agent à énacter une interaction de déplacement. 
...
Lorsque le lieu courant est connue, l'agent passe en état émotionnel \texttt{enjoyment} qui le conduisent à choisir des décision \texttt{move} dans des directions ou il n'y a pas d'obstacles, ce qui conduit à un comportement d'exploration. 
...
La notion de \an{bundle} est étendue à une structure de données qui indique dans quelle configuration spatiales une interactions affordée par l'objet peut être énactées. 
...
Tout comme un algorithme de SLAM, un algorithme de SLAPI fusionne des informations permettant de localiser l'objet avec un calcul d'intégration des déplacements du robot. 
...
L'artcile \an{Reducing intuitive physics prediction error through playing} \cite{georgeon_reducing_2024}, co-écrit avec Béatrice de Montéra et Paul Robertson rapport comment ce signal d'erreur de prédiction permet à Patitcat de calibrer automatiquement certains des paramètres prédéfinis dans ECA.

\subsection{Enseignements (en construction)}
...
Par exemple l'algorithe SLAPI génère des objectifs à atteindre pour placer le robot dans différents position autour d'un objet pour tester les possibilités d'interaction offertes par cet objet sous différents angles. 
...
Pour l'instant ces deux mécanismes sont séparés et interagissent à travers fonction ad-hoc. Ils produisent des schémas stockés séparément.
...
Beaucoup de ces paramètres sont liés au modèle corporel du robot et à son modèle de déplacement, par exemple la vitesse de locomotion, la distance de retrait en cas de détection d'une ligne noire, la position des capteurs, la vitesse du son utilisée par le télémètre, etc . 
Nous avons mis en place des méthodes pour permettre au robot de calibrer ces paramètres sur la base de comportements autonomes (Section \ref{sec:slapi}) mais elles nécessite une initialisation de leur valeur approximative. 
...
Ces décisions implémentent une heuristique de recherche de fiabilité qui tend à informer l'agent sur sa situation dans l'environnement. 
...
\begin{figure}[ht]
	...
	\caption{Perspective de trajectoire développementale.}
