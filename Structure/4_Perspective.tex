...
\noindent
Avec Béatrice De Montéra et Paul Robertson, nous avons défini le problème de la \emph{Localisation et Inférence Active simultanée de Phénomène}  \cite[\an{Simultaneous Localisation and Active Phenomenon Inference}]{georgeon_simultaneous_2022} (SLAPI).
Ce problème se réfère au problème bien connu en robotique du \an{Simultaneous localisation and Mapping} (SLAM) mais il en diffère par le fait qu'il se limite à permettre au robot de se localiser dans le voisinage d'un objet tout en construisant un modèle mental de cet objet.
...
L'agent représente ainsi l'objet par le \an{bundle} des interactions qu'il afford organisées spatialement. 
Nous utilisons le terme \emph{phénomène} pour désigner la structure de donnée constituée du \an{bundle} d'interaction spatialement organisées qui représente l'objet pour l'agent. 
