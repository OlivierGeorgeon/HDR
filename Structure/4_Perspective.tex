...
\subsection{Expérimentation des \an{Place cells}}

\noindent
Nous avons doté ECA de la capacité de construire un réseau de \emph{places} en mémoire allocentrique. 
Cette capacité s'inspire des \emph{cellules de place} (\an{place cells}) dans le cerveau.
L'hypothèse est qu'un animal est capable d'adapter ses comportements à une configuration particulière de l'environnement observé à une certaine position.
Il peut ensuite mémoriser des possibilités de déplacement entre ces différentes positions, et reconnaitre une position qui a déjà visitée. 

Nous avons implémenté un comportement qui 
