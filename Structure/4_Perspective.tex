...
\subsubsection{Construction d'une mémoire spatiale à partir d'événements de flux optique}
\label{sec:simon}

\noindent
Simon Gay a proposé un mécanisme de construction d'une mémoire spatiale en exploitant capteur de flux optique \cite[\an{Autonomous construction and exploitation of a spatial memory by a self-motivated agent}]{gay_autonomous_2017}.

Cet agent dispose de cinq décisions: \texttt{move forward}, \texttt{turn left 90}, \texttt{turn right 90}, \texttt{turn left 45},  \texttt{turn right 45}. 
...
La Figure~\ref{fig:robot_simon} montre les conditions expérimentales utilisant un robot Lego dans un environnement similaire à l'expérimentation des requins Section~\ref{sec:requins}.
Le robot contient un capteur de collision, un capteur de couleur situé sous lui, et une caméra générant alimentant traitement générant des événements de flux optique.
...
\begin{figure}[ht]
	...
	\caption{Johnny le robot Lego \cite{gay_autonomous_2014}.
	Gauche: robot disposant d'une caméra orientée vers un miroir sphérique offrant une vision panoramique de l'environnement. 
	Centre: traitement d'image générant des flux optiques de points saillants.
	Droite: robot dans l'environnement répliquant l'expérimentation des requins.}
