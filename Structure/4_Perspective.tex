...
A l'usage, cependant, la plateforme ROS s'est montrée inutilement compliquée pour mettre en \oe uvre le modèle SEDP. La présence d'un PC embarqué s'est avérée également inutile.
% car il est plus pratique d'implémenter ECA sur un PC distant. communiquant avec le robot par wifi.

Nous avons finalement opté pour un robot basé sur la plateforme illustrée décrite en Section \ref{sec:petitcat_hardware}. 
...
La carte carte Arduino controle l'exécution de la décision reçue par wifi et renvoie les événements d'interaction . 
...
\begin{figure}[ht]
	...
	\caption{Robot Petitcat adapté de \cite[Fig.~7 et 8]{georgeon_artificial_2024}.
		La carte Arduino contrôle l'énaction des décisions en commandant les effecteurs et capteurs: roues omnidirectionnelles, rotation de la tête, télémètre à ultrason, barre de capteurs de luminosité du sol, capteur de couleur du sol, carte inertielle, LED multicolore, puis renvoie les événements au PC via wifi. 
	} 
...
Le robot dispose des interactions:
translate
swipe 
turn
turn-head
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|c|}
		...
		\texttt{translate} & translation longitudinale & durée (ms) \\ \hline
		\texttt{swipe} & translation latérale  & durée (ms) \\ \hline
	...
	\end{tabular}
	\caption{Décisions primitives de PetitCat.
	}
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|c|c|}
		...
		\texttt{clock} & obligatoire & toutes & numéro du cycle d'interaction \\ \hline
		\texttt{decision code} & obligatoire  & toutes & code de la décision \\ \hline
		\texttt{focus-x} & optionnel & \texttt{translate}, \texttt{swipe}, \texttt{turn} & coordonnée x du point d'attention \\ \hline
		\texttt{focus-y} & optionnel & \texttt{translate}, \texttt{swipe}, \texttt{turn} & coordonnée y du point d'attention \\ \hline
		\texttt{color} & optionnel & toutes & couleur de la LED\\ \hline
		\texttt{duration} & optionnel & \texttt{translate}, \texttt{swipe} & durée \\ \hline
		\texttt{angle} & optionnel & \texttt{turn}, \texttt{turn-head}, \texttt{scan} & angle \\ \hline
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		...
		\texttt{clock} & numéro du cycle d'interaction renvoyé pour vérification \\ \hline
	...
	\end{tabular}
	\caption{Variables associées aux événements d'interaction.
	}
