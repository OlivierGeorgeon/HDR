...
Les variables \texttt{focus-x} et \texttt{focus-y} permettent de définir les coordonnées d'un point dans le référentiel égocentrique de Petitcat. 
Quand elles sont spécifiées, Petitcat maintient sa tête orientée vers ce point pendant tout la durée de la décision, ce qui lui permet de suivre la position d'un objet fixe, et suggère à un observateur extérieur que Petitcat maintient son attention sur cet objet. 
...
Le tableau \ref{tab:interactions_petitcat} liste les variables renvoyées par Petitcat à ECA après l'enaction de la décision. 
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		...
		\texttt{head angle} & direction de la tête (degrés) \\ \hline
		...
		\texttt{azimuth} & direction par rapport au nord, mesurée de la boussole (degrés) \\ \hline
		\texttt{impact} & code d'impact mesuré par l'unité de mesure inertielle  \\ \hline
