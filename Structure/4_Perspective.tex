...
L'objectif est de générer des comportements qu'un observateur humain peut juger naturel et intéressant. 
Nous examinons comment cet objectif pragmatique de comportements naturels impacte la conception d'une architecture cognitive basée sur la théorie des Evénements d'Interaction Valués. 
En particulier nous avons été amenés à associer des variables libres aux décisions et aux interactions, rejoignant ainsi des choix de conception pris par d'autres architectures cognitives construcgtivistes examinées en Section~\ref{sec:architectures}.
...
La carte Arduinon Mega s'avère plus simple qu'un rasberry pour implémenter le contrôleur primitif des interactions. Elle permet un cycle d'interaction à la vitesse très fiable de l'ordre de la dizaine de millisecondes. 
...
La centrale inertielle contient un accéléromètre linéaire, un gyromètre et une boussole. 
...
Petitcat peut exécuter les sept décisions listées dans le Tableau~\ref{tab:decision_petitcat}, qui peuvent être paramétrées par les variables libres du Tableau~\ref{tab:parametres_petitcat}.
...
Petitcat ne disposant pas d'odomètre, la spécification de la distance parcourues pendant les décisons \texttt{move} et \texttt{swipe} se fait par la variable \texttt{duration}.
La vitesse approximative est de $300 mm/s$. 
...
L'angle de rotation associé à la décision \texttt{turn} est contrôle par le gyromètre de la centrale inertielle. 
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|c|c|}
		...
		\texttt{duration} & optionnel & \texttt{move}, \texttt{swipe} & durée \\ \hline
